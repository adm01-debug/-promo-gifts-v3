...
export default function ProductDetail() {
  ...
  useEffect(() => {
    ...
    let match = product.variations.find(
      (v: any) => v.color?.name?.toLowerCase().trim() === normalizedParam,
    );
    ...
    if (!match && hexParam)
      match = product.variations.find(
        (v: any) => v.color?.hex?.toLowerCase() === hexParam.toLowerCase(),
      );
    if (!match && grupoParam && product.colors?.length) {
      const c = product.colors.find((c: any) => c.groupSlug === grupoParam);
      if (c)
        match = product.variations.find(
          (v: any) => v.color?.name?.toLowerCase() === c.name?.toLowerCase(),
        );
    }
