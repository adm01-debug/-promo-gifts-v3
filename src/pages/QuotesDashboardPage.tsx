            ...
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xl font-bold text-warning">{s.metrics.pendingQuotes}</p>
                </div>
                ...
                {[
                  {
                    ...
                    value: s.tokenStats.total,
                    label: 'Enviados',
                    pct: null,
                  },
                  {
                    icon: <Eye className="h-4 w-4 text-warning" />,
                    value: s.tokenStats.viewed,
                    label: 'Visualizados',
                    pct:
                      s.tokenStats.total > 0
                        ? ((s.tokenStats.viewed / s.tokenStats.total) * 100).toFixed(0)
                        : '0',
                  },
                  {
                    icon: <CheckCircle className="h-4 w-4 text-success" />,
                    value: s.tokenStats.responded,
                    label: 'Respondidos',
                    pct:
                      s.tokenStats.viewed > 0
                        ? ((s.tokenStats.responded / s.tokenStats.viewed) * 100).toFixed(0)
                        : '0',
                  },
                ].map((t) => (
                  <div key={t.label} className="rounded-lg bg-muted/30 p-3 text-center">
                    <div className="mb-1 flex items-center justify-center gap-1.5">{t.icon}</div>
                    <p className="text-xl font-bold text-foreground">{t.value}</p>
                    <p className="text-xs text-muted-foreground">{t.label}</p>
                    {t.pct !== null && (
                      <p className="text-xs text-muted-foreground/70">({t.pct}%)</p>
                    )}
                  </div>
                ))}
              </div>
