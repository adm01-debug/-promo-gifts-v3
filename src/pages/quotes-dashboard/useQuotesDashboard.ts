...
export function useQuotesDashboard() {
  ...
  const quotesClients = useMemo(() => {
    ...
    quotes.forEach(q => { if (q.client_id && q.client_name) map.set(q.client_id, q.client_name); });
    return Array.from(map, ([id, name]) => ({ id, name }));
  }, [quotes]);

  const selectedClientName = useMemo(() => {
    if (selectedClientId === "all") return null;
    return clients.find(c => c.id === selectedClientId)?.name || quotesClients.find(c => c.id === selectedClientId)?.name || null;
  }, [selectedClientId, clients, quotesClients]);

  const metrics = useMemo(() => {
    if (!quotes.length) return {
      totalQuotes: 0, totalValue: 0, approvedValue: 0, approvalRate: 0, rejectionRate: 0,
      averageResponseTime: 0, averageValue: 0, pendingQuotes: 0,
      statusDistribution: [] as { name: string; value: number; color: string }[],
      monthlyData: [] as { month: string; total: number; approved: number; rejected: number; value: number }[],
    ...
    switch (selectedPeriod) {
      ...
      case "year": startDate = subMonths(startOfMonth(now), 11); break;
    }

    let filtered = quotes.filter(q => new Date(q.created_at) >= startDate);
    if (selectedClientId !== "all") filtered = filtered.filter(q => q.client_id === selectedClientId);
    ...
    const withResponse = filtered.filter(q => q.client_response_at && (q.status === "approved" || q.status === "rejected"));
    let averageResponseTime = 0;
    if (withResponse.length > 0) {
      averageResponseTime = withResponse.reduce((s, q) => s + differenceInHours(new Date(q.client_response_at!), new Date(q.created_at)), 0) / withResponse.length;
    }

    const statusCounts = filtered.reduce((a, q) => { a[q.status] = (a[q.status] || 0) + 1; return a; }, {} as Record<string, number>);
    const statusDistribution = Object.entries(statusCounts).map(([status, count]) => ({
      name: statusConfig[status]?.label || status, value: count,
      color: statusConfig[status]?.color || "hsl(var(--muted))",
    }));

    const monthlyGroups = filtered.reduce((a, q) => {
      const m = format(new Date(q.created_at), "MMM", { locale: ptBR });
      if (!a[m]) a[m] = { month: m, total: 0, approved: 0, rejected: 0, value: 0 };
      a[m].total++;
      if (q.status === "approved") { a[m].approved++; a[m].value += q.total || 0; }
    ...
    const conversionFunnel = [
      ...
      { stage: "Enviados", count: filtered.filter(q => ["sent", "approved", "rejected"].includes(q.status)).length, fill: "hsl(var(--info))" },
      { stage: "Respondidos", count: responded.length, fill: "hsl(var(--warning))" },
      { stage: "Aprovados", count: approved.length, fill: "hsl(var(--success))" },
    ];

    return {
      totalQuotes, totalValue, approvedValue, approvalRate, rejectionRate,
      averageResponseTime, averageValue: totalQuotes > 0 ? totalValue / totalQuotes : 0,
      pendingQuotes: pending.length, statusDistribution,
      monthlyData: Object.values(monthlyGroups), conversionFunnel,
    };
  ...
  const exportToPdf = useCallback(() => {
    ...
    doc.setFontSize(18); doc.text("Dashboard de Conversão de Orçamentos", 14, 20);
    ...
    autoTable(doc, {
      ...
      theme: "grid", headStyles: { fillColor: [245, 158, 11] }, styles: { fontSize: 10 },
    });

    if (metrics.statusDistribution.length > 0) {
      const y = doc.lastAutoTable?.finalY || 80;
      doc.setFontSize(13); doc.text("Distribuição por Status", 14, y + 12);
      autoTable(doc, { startY: y + 16, head: [["Status", "Quantidade"]],
    ...
    if (metrics.conversionFunnel.length > 0) {
      ...
      doc.setFontSize(13); doc.text("Funil de Conversão", 14, y + 12);
      autoTable(doc, { startY: y + 16, head: [["Etapa", "Quantidade"]],
    ...
    if (tokenStats.total > 0) {
      ...
      doc.setFontSize(13); doc.text("Links de Aprovação", 14, y + 12);
      autoTable(doc, { startY: y + 16, head: [["Métrica", "Valor"]],
        body: [
    ...
    if (recentResponses.length > 0) {
      ...
      doc.setFontSize(13); doc.text("Últimas Respostas de Clientes", 14, startY);
      autoTable(doc, { startY: startY + 4, head: [["Orçamento", "Status", "Data", "Valor"]],
        ...
        theme: "grid", headStyles: { fillColor: [245, 158, 11] }, styles: { fontSize: 9 } });
    }
  ...
  return {
    quotes, isLoading, selectedPeriod, setSelectedPeriod,
    selectedClientId, setSelectedClientId, clients, quotesClients,
    loadingClients, tokenStats, selectedClientName, metrics,
    exportToPdf,
  };
}
