    ...
    if (tokenStats.total > 0) {
      ...
      autoTable(doc, {
        ...
        body: [
          ...
          [
            'Visualizados',
            `${tokenStats.viewed} (${tokenStats.total > 0 ? ((tokenStats.viewed / tokenStats.total) * 100).toFixed(0) : 0}%)`,
          ],
          [
            'Respondidos',
            `${tokenStats.responded} (${tokenStats.viewed > 0 ? ((tokenStats.responded / tokenStats.viewed) * 100).toFixed(0) : 0}%)`,
          ],
        ],
        theme: 'grid',
        headStyles: { fillColor: [245, 158, 11] },
