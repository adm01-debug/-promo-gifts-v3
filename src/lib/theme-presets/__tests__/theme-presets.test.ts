...
describe('Theme Presets Consistency & Contrast', () => {
  it('should not override default fonts in any preset', () => {
    THEME_PRESETS.forEach((preset) => {
      expect(
        preset.font,
        `Preset "${preset.name}" (${preset.id}) is overriding the default font stack.`,
      ).toBeUndefined();
    });
  });
  ...
  it('should have basic color tokens defined in both light and dark modes', () => {
    ...
    THEME_PRESETS.forEach((preset) => {
      requiredTokens.forEach((token) => {
        expect(
          preset.light[token as keyof typeof preset.light],
          `Preset "${preset.name}" is missing "${token}" in light mode.`,
        ).toBeDefined();
        expect(
          preset.dark[token as keyof typeof preset.dark],
          `Preset "${preset.name}" is missing "${token}" in dark mode.`,
        ).toBeDefined();
      });
    });
  });
  ...
  it('should maintain WCAG contrast ratios for key text elements', () => {
    THEME_PRESETS.forEach((preset) => {
      ...
      expect(
        lightContrast,
        `Preset "${preset.name}" (Light) background/foreground contrast: ${lightContrast.toFixed(2)}:1`,
      ...
      expect(
        lightCardContrast,
        `Preset "${preset.name}" (Light) card contrast: ${lightCardContrast.toFixed(2)}:1`,
      ).toBeGreaterThanOrEqual(4.5);
      ...
      expect(
        darkContrast,
        `Preset "${preset.name}" (Dark) background/foreground contrast: ${darkContrast.toFixed(2)}:1`,
      ).toBeGreaterThanOrEqual(4.5);
      ...
      expect(
        darkCardContrast,
        `Preset "${preset.name}" (Dark) card contrast: ${darkCardContrast.toFixed(2)}:1`,
      ).toBeGreaterThanOrEqual(4.5);
      ...
      expect(
        primaryContrast,
        `Preset "${preset.name}" (Light) primary button contrast: ${primaryContrast.toFixed(2)}:1`,
      ...
      expect(
        darkPrimaryContrast,
        `Preset "${preset.name}" (Dark) primary button contrast: ${darkPrimaryContrast.toFixed(2)}:1`,
      ).toBeGreaterThanOrEqual(3);
    });
  });
