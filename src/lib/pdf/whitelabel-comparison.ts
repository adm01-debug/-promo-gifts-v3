...
export async function exportWhitelabelComparisonPDF(opts: {
  ...
  pdf.setFontSize(13);
  pdf.text(
    branding?.name ? `Comparação de Produtos — ${branding.name}` : "Comparação de Produtos — Promo Gifts",
    10, 9
  );
  pdf.setFontSize(8);
  ...
  if (imgHeight < pageHeight - 25) {
    ...
    while (remaining > 0) {
      ...
      if (remaining > 0) { pdf.addPage(); position = 10; }
    }
  }
