...
async function enrichProducts(products: PromobrindProduct[], options?: { limit?: number }) {
  ...
  for (const idx of queryMap.suppliers) {
    ...
    if (r?.success && r.data?.records)
      suppliersRecords.push(...(r.data.records as ExternalProduct[]));
  }
  ...
  const colorVariationMap = new Map(
    colorVariationsRecords.map((v) => [
      v.id as string,
      { name: v.name as string, slug: v.slug as string, group_id: v.group_id as string },
    ]),
  );
  const colorGroupMap = new Map(
    colorGroupsRecords.map((g) => [
      g.id as string,
      { name: g.name as string, slug: g.slug as string },
    ]),
  );
