...
export function useProductRegistration() {
  ...
  const loadReferenceData = useCallback(async () => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : 'Erro ao carregar dados de referência';
  ...
  const { importProgress, importProducts, validateImportRow, generateTemplate } = useProductImport(
    referenceData,
    createProduct,
  );
