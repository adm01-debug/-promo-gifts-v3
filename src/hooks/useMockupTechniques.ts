    ...
    queryFn: async () => {
      ...
      try {
        ...
        if (cached) {
          ...
          return new Map(Object.entries(parsed)) as Map<string, { maxWidth: number | null; maxHeight: number | null }>;
        }
      } catch (e) { /* ignore */ }
      ...
      for (const tech of techResult.records) {
        ...
        let lwS = false, lhS = false;

        for (const f of faixas) {
          if (f.largura_max != null) { if (f.largura_max >= 90) lwS = true; else larguras.push(f.largura_max); }
          if (f.altura_max != null) { if (f.altura_max >= 90) lhS = true; else alturas.push(f.altura_max); }
        }
      ...
      try { sessionStorage.setItem(CACHE_KEY, JSON.stringify(Object.fromEntries(codeMap))); } catch (e) { /* ignore */ }
      return codeMap;
    },
    enabled: shouldFetch,
...
export function useFilteredTechniques(
  ...
  const { data: customizationData, isFetching } = useProductCustomizationOptionsForMockup(selectedProduct?.id);
  ...
  return useMemo(() => {
    if (!selectedProduct || !techniques.length) {
      return techniques.map(t => ({
        ...t, maxWidth: null, maxHeight: null, areaName: null, locationName: null,
        maxColors: null, chargesPerColor: false, usesDimension: false, isCurved: false,
        setupCost: null, variationLabel: null, groupCode: null, shape: null,
      }));
    }
    ...
    if (customizationData?.locations && customizationData.locations.length === 0) {
      return techniques.map(t => {
        ...
        return {
          ...t, maxWidth: dims?.maxWidth ?? null, maxHeight: dims?.maxHeight ?? null,
          areaName: null, locationName: null, maxColors: null, chargesPerColor: false,
          usesDimension: false, isCurved: false, setupCost: null, variationLabel: null,
          groupCode: null, shape: null,
        };
      });
    }
    ...
    if (customizationData?.locations) {
      for (const location of customizationData.locations) {
        for (const option of location.options) {
          ...
          const existingArea = existing ? (existing.option.efetiva_largura_max || 0) * (existing.option.efetiva_altura_max || 0) : 0;
          if (!existing || area > existingArea) techniqueMap.set(key, { option, locationName: location.location_name });
        }
      }
    }

    const result: TechniqueWithLimits[] = [];
    for (const [_, { option, locationName }] of techniqueMap.entries()) {
      result.push({
        id: option.technique_id, name: option.tecnica_nome, code: option.codigo_tabela || null,
        maxWidth: option.efetiva_largura_max || null, maxHeight: option.efetiva_altura_max || null,
        areaName: `${locationName} — ${option.tecnica_nome}`, locationName,
        maxColors: option.max_cores ?? null, chargesPerColor: option.cobra_por_cor ?? false,
        usesDimension: option.usa_dimensao ?? false, isCurved: option.is_curved ?? false,
        setupCost: option.custo_setup ?? null, variationLabel: option.variacao_label || null,
        groupCode: option.grupo_tecnica || null, shape: option.shape || null,
      });
    }
