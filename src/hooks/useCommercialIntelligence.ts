...
import {
  ...
  aggregateSegments,
  aggregateClients,
} from './intelligence/intelligenceHelpers';
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<TrendingProduct[]> => {
      ...
      quoteItems?.forEach((item) => {
        const key = item.product_sku || item.product_id || item.product_name;
        if (key && productMap.has(key)) productMap.get(key)!.quoteCount += 1;
      });
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<RevenuePoint[]> => {
      ...
      if (hasFilter && productIds) {
        const pids = Array.from(productIds).slice(0, 200);
        if (!pids.length) {
          const m = new Map<string, RevenuePoint>();
          for (let i = 0; i < days; i++) {
            const d = new Date(since);
            d.setDate(d.getDate() + i);
            const k = d.toISOString().split('T')[0];
            m.set(k, { date: k, revenue: 0, orders: 0, quotes: 0 });
          }
      ...
      for (let i = 0; i < days; i++) {
        ...
        d.setDate(d.getDate() + i);
        const k = d.toISOString().split('T')[0];
        dateMap.set(k, { date: k, revenue: 0, orders: 0, quotes: 0 });
      }
      orderData.forEach((o) => {
        const k = new Date(o.created_at).toISOString().split('T')[0];
        const e = dateMap.get(k);
        if (e) {
          e.revenue += (o.quantity ?? 1) * (o.unit_price ?? 0);
          e.orders += 1;
        }
      });
      quoteData.forEach((q) => {
        const k = new Date(q.created_at).toISOString().split('T')[0];
        const e = dateMap.get(k);
        if (e) e.quotes += 1;
      });
