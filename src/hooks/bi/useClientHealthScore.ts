...
export function useClientHealthScore(
  ...
  return useMemo(() => {
    ...
    const sharePct = industryAnnualPotential > 0
    ...
    let scriptHint = "Olá! Passando para alinhar próximas demandas — vi que estamos no seu mês de pico histórico.";
    let ctaUrgent = false;
    ...
    if (recScore <= 30) {
      ...
      scriptHint = "Sentimos sua falta! Preparei uma seleção exclusiva baseada nos produtos que você mais gosta.";
      ctaUrgent = true;
    } else if (seas.daysToNextPeak !== null && seas.daysToNextPeak <= 30) {
      nextActionLabel = seas.daysToNextPeak === 0 ? "Abordar agora · pico" : `Abordar em ${seas.daysToNextPeak}d`;
      nextActionDetail = `Cliente entra em mês de pico em breve — momento ideal para envio de orçamento.`;
      scriptHint = "Estamos na sua janela histórica de compra — separei os destaques do setor para você avaliar.";
      ctaUrgent = true;
      ...
      scriptHint = "Notei que ainda não trabalhamos algumas linhas que são padrão no seu setor — posso te mostrar?";
      ...
      scriptHint = "Acabou de chegar uma linha premium que combina com o perfil de compra que você costuma escolher.";
    }
