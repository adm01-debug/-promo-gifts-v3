...
export function useTechniqueRecommendations(
  ...
  const recommendedTechniques = useMemo(() => {
    ...
    return techniques.map((technique): TechniqueWithRecommendation => {
      const { score, matchedKeywords, reason } = calculateRecommendationScore(
        technique,
        keywords
      );
      
      const popularityKey = Object.keys(POPULARITY_SCORES).find(
        k => (technique.code?.toUpperCase() || '').includes(k)
      );
      const popularityScore = popularityKey ? POPULARITY_SCORES[popularityKey] : 30;
...
export type SortOption = 'recommended' | 'price_asc' | 'price_desc' | 'time_asc' | 'time_desc' | 'popularity';
