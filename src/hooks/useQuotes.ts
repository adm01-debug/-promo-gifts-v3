...
export function useQuotes(filters: QuotesListFilters = {}) {
  ...
  const fetchQuotes = async () => {
    ...
    try {
      ...
      if (search) {
        q = q.or(
          `quote_number.ilike.%${search}%,client_name.ilike.%${search}%,client_company.ilike.%${search}%`,
        );
      }
      ...
      const message =
        err instanceof Error ? err.message : (err as any)?.message || 'Erro ao buscar orçamentos';
      setError(message);
      ...
      await Promise.allSettled(
        quoteIds.map((quoteId) =>
          logQuoteHistory(
            quoteId,
            'status_changed',
            `Status alterado em massa para ${STATUS_LABELS[status] || status}`,
          ),
        ),
      );
  ...
  const updateQuote = async (
    ...
    try {
      ...
      if (totals.markup > 0 && totals.subtotal < totals.realSubtotal) {
        throw new Error(
          'Falha na integridade dos cálculos: subtotal apresentado menor que subtotal real.',
        );
      }
