...
export function useRamoAtividadeFilter(): UseRamoAtividadeFilterReturn {
  ...
  const filteredSegmentos = useMemo(() => {
    ...
    if (filterState.searchTerm) {
      ...
      result = result.filter(
        (s) =>
          s.segmento_name.toLowerCase().includes(term) || s.ramo_name.toLowerCase().includes(term),
      );
    }
  ...
  const activeFiltersCount =
    filterState.selectedRamos.length + filterState.selectedSegmentos.length;
