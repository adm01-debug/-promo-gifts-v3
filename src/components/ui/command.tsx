...
>(({ className, ...props }, ref) => (
  <CommandPrimitive
    ref={ref}
    className={cn(
      'flex h-full w-full flex-col overflow-hidden rounded-xl text-popover-foreground [background-color:hsl(var(--command-surface))]',
      className,
    )}
    {...props}
  />
...
const CommandDialog = ({ children, ...props }: CommandDialogProps) => {
  return (
    <Dialog {...props}>
      <DialogContent
        className="max-w-[560px] overflow-hidden !rounded-xl bg-[hsl(var(--command-surface))] p-0 shadow-[0_24px_80px_hsl(var(--command-shadow))] [border-color:hsl(var(--command-border))] [&>div]:overflow-hidden"
...
>(({ className, ...props }, ref) => (
  ...
  >
    ...
    <CommandPrimitive.Input
      ref={ref}
      className={cn(
        'flex h-11 w-full rounded-xl bg-transparent py-3 text-sm outline-none ring-0 placeholder:text-muted-foreground focus:outline-none focus:ring-0 disabled:cursor-not-allowed disabled:opacity-50',
        className,
      )}
      {...props}
    />
