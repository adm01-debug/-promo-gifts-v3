    ...
    <div className="mb-1 pb-3">
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-gradient-to-br from-orange to-orange-hover shadow-md shadow-orange/20">
        ...
        {activeFiltersCount > 0 && (
          <span className="inline-flex h-7 min-w-7 animate-scale-in items-center justify-center rounded-full bg-orange px-2 text-xs font-bold text-orange-foreground shadow-sm shadow-orange/30">
            {activeFiltersCount}
          </span>
        )}
      </div>
      ...
      <div className="relative mt-3">
        <Search className="absolute left-2.5 top-1/2 z-10 h-3.5 w-3.5 -translate-y-1/2 text-orange" />
        <Input
          placeholder="Buscar filtro..."
          value={filterSearch}
          onChange={(e) => setFilterSearch(e.target.value)}
          className="h-8 border-border/40 bg-muted/30 pl-8 pr-8 text-xs placeholder:text-muted-foreground/40 focus:border-orange/50"
