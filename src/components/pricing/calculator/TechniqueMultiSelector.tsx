...
export function TechniqueMultiSelector({ productId, selectedTechniques, onToggleTechnique }: Props) {
  const { data: techniques, isLoading, error } = useQuery({
    ...
    queryFn: async (): Promise<ProductTechnique[]> => {
      try {
        const result = await invokeExternalRpc<any>('fn_get_product_customization_options', { p_product_id: productId });
  ...
  if (error) return <div className="p-4 rounded-md bg-destructive/10 border border-destructive/20 text-destructive"><AlertCircle className="w-5 h-5 mb-2" /><p>Erro ao carregar técnicas</p></div>;
            ...
            {techs.map(tech => {
              ...
              return (
                <button key={tech.id} onClick={() => onToggleTechnique(tech, !selected)}
                  className={cn("p-3 rounded-md border text-left transition-all", selected ? "bg-primary/10 border-primary ring-1 ring-primary" : "bg-card hover:bg-accent border-border")}>
