...
import type { CalcProduct, SelectedTechniqueConfig } from './types';
import { availableSizes, formatCurrency, formatNumber } from './types';
...
export function QuantityComparisonTable({ product, selectedConfigs, quantities }: Props) {
  ...
  return (
    <div className="space-y-4">
            ...
            {selectedConfigs.map((config) => (
              <TableRow key={config.technique.id}>
                ...
                {quantities.map(qty => {
                  ...
                  return (
                    <TableCell key={qty} className={cn("text-center relative", isBest && "bg-success/10 border border-success/30 rounded-2xl")}>
      ...
      <div className="text-xs text-muted-foreground p-3 bg-muted/50 rounded-2xl">
