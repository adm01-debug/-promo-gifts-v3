        ...
        <div className="space-y-4">
          ...
          <AnimatePresence mode="wait">
            {isCreating ? (
              ...
              >
                ...
                >
                  {/* Mini preview */}
                  <div className="flex items-center gap-2 p-2 rounded-2xl bg-background/80 border border-border/50">
                    <motion.div
                      key={`${selectedColor}-${selectedIcon}`}
                      initial={{ scale: 0.8, rotate: -10 }}
                      animate={{ scale: 1, rotate: 0 }}
                      transition={{ type: "spring", stiffness: 500, damping: 25 }}
                      className="w-8 h-8 rounded-2xl flex items-center justify-center text-sm"
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {defaultIcons.map((icon) => (
                        <motion.button
                          ...
                          className={cn(
                            "w-9 h-9 rounded-2xl text-base flex items-center justify-center border transition-all",
                            selectedIcon === icon
          ...
          {collectionsWithProduct.length > 0 && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="flex items-center gap-2 px-3 py-2 rounded-2xl bg-muted/40 border border-border/50"
            >
