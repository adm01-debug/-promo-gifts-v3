...
export function RestrictedRouteNotice({ isCollapsed }: RestrictedRouteNoticeProps) {
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <div
              ...
              className={cn(
                'mx-auto my-2 flex h-8 w-8 items-center justify-center rounded-xl',
          ...
          <TooltipContent side="right" className="max-w-xs">
            ...
            <p className="mt-1 text-[11px]">
              Papel exigido: <span className="font-mono font-semibold">{requiredRole}</span>
            </p>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
  ...
  return (
    <div
      ...
      className="mx-2 my-2 rounded-xl border border-destructive/30 bg-destructive/10 p-2.5 text-xs"
    >
      <div className="flex items-start gap-2">
        <ShieldAlert className="mt-0.5 h-4 w-4 flex-shrink-0 text-destructive" aria-hidden="true" />
        <div className="min-w-0 flex-1">
          <p className="font-semibold leading-tight text-destructive">{title}</p>
          <p className="mt-0.5 leading-snug text-destructive/80">{description}</p>
