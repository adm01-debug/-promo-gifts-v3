                ...
                <Card key={cart.id} className="p-4 space-y-3">
                  <div className="flex items-center gap-2">
                    {cart.company_logo_url ? (
                      <img src={cart.company_logo_url} alt="Logo da empresa" className="w-8 h-8 rounded-2xl object-contain bg-background border border-border/50 p-0.5" loading="lazy" />
                    ) : (
                  ...
                  <div className="space-y-1.5 max-h-48 overflow-y-auto">
                    {cart.items.map(item => (
                      <div key={item.id} className="flex items-center gap-2 text-xs p-1.5 rounded-2xl bg-muted/30">
