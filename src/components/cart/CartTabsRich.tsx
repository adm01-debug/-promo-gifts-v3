    ...
    <div className="scrollbar-none flex snap-x snap-mandatory gap-2.5 overflow-x-auto px-1 pb-2">
      {carts.map((cart) => {
        ...
        return (
          <button
            ...
            className={cn(
              'group relative flex flex-shrink-0 snap-start items-center gap-3 whitespace-nowrap rounded-xl border px-4 py-2.5 transition-all duration-500 animate-in fade-in slide-in-from-left-4',
              isActive
          ...
          >
            ...
            {cart.company_logo_url ? (
              <img
                ...
                alt=""
                className="h-8 w-8 flex-shrink-0 rounded-xl border border-border/40 bg-background object-contain p-1 transition-transform group-hover:scale-110"
                loading="lazy"
              />
            ) : (
              <div
            ...
            <div className="flex flex-col items-start gap-0.5 leading-none">
              ...
              <div className="flex items-center gap-2 opacity-80">
                ...
                {isCreatedToday && (
                  <Badge
                    variant="outline"
                    className="animate-pulse-slow h-4 border-success/20 bg-success/10 px-1.5 text-[8px] font-black uppercase text-success"
                  >
                    Hoje
                  </Badge>
                )}
      ...
      {canCreateCart && (
        <button
          ...
          onClick={onNew}
          className={cn(
            'group/new flex flex-shrink-0 items-center gap-2 rounded-xl border-2 border-dashed border-border/40 px-5 py-2.5 transition-all',
        ...
        >
          <div className="flex h-6 w-6 items-center justify-center rounded-xl bg-muted/40 transition-colors group-hover/new:bg-primary/20">
