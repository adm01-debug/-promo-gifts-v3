  ...
  if (isLoading) {
    return (
      <div className="flex gap-2 overflow-x-auto pb-1 animate-pulse">
        {[...Array(3)].map((_, i) => (
          <div key={i} className="flex items-center gap-2.5 px-3.5 py-2 rounded-lg border border-border/30 bg-muted/5 w-[180px] flex-shrink-0">
    ...
    <div className="flex gap-2.5 overflow-x-auto pb-2 scrollbar-none snap-x snap-mandatory px-1">
      {carts.map(cart => {
        ...
        return (
          <button
            ...
            className={cn(
              "group relative flex items-center gap-3 px-4 py-2.5 rounded-lg border transition-all whitespace-nowrap flex-shrink-0 animate-in fade-in slide-in-from-left-4 duration-500 snap-start",
              isActive
          ...
          >
            ...
            {cart.company_logo_url ? (
              <img src={cart.company_logo_url} alt="" className="w-8 h-8 rounded-lg object-contain bg-background border border-border/40 p-1 flex-shrink-0 group-hover:scale-110 transition-transform" loading="lazy" />
            ) : (
              <div className={cn(
      ...
      {canCreateCart && (
        <button
          ...
          onClick={onNew}
          className={cn(
            "flex items-center gap-2 px-5 py-2.5 rounded-lg border-2 border-dashed border-border/40 transition-all flex-shrink-0 group/new",
