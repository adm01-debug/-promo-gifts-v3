...
export function CartCompanyPickerDialog({
  ...
  const renderRow = (company: CompanyItem) => (
    <div
      key={company.id}
      className={cn(
        'group flex w-full items-center gap-2 rounded-xl px-1 transition-colors',
        'hover:bg-accent/60',
      )}
    >
      <button
      ...
      >
        {company.logo_url ? (
          <img
            ...
            alt=""
            className="h-9 w-9 flex-shrink-0 rounded-xl border border-border/40 bg-background object-contain"
            loading="lazy"
          />
        ) : (
      ...
      <button
        ...
        className={cn(
          'mr-1 flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-xl outline-none transition-colors focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-1',
          isFavorite(company.id)
            ? 'text-warning'
            : 'text-muted-foreground/40 opacity-0 hover:text-warning focus-visible:opacity-100 group-hover:opacity-100',
        )}
                ...
                <div className="space-y-1 py-1">
                  {[...Array(4)].map((_, i) => (
                    <div key={i} className="flex items-center gap-3 rounded-xl px-3 py-2.5">
                      <Skeleton className="h-9 w-9 rounded-xl opacity-20" />
            ...
            <div className="px-2" role="search">
              <div className="relative">
                ...
                <Input
                  ...
                  className="h-9 border-border/40 bg-muted/20 pl-8 pr-8 text-sm transition-colors focus:bg-background focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-1"
                  aria-busy={isLoading}
                ...
                {isLoading && (
                  <>
                    <Loader2
                      ...
                      className="pointer-events-none absolute right-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 animate-spin text-muted-foreground opacity-50"
                    />
                    <span className="sr-only">Carregando empresas...</span>
                  </>
                )}
                <div className="sr-only" aria-live="polite" id="search-announcement">
                  {searchTerm.length >= 1 &&
                    !isLoading &&
