...
export function CartCompanyPickerDialog({ open, onOpenChange, onCreated }: CartCompanyPickerDialogProps) {
  ...
  const renderRow = (company: CompanyItem) => (
    ...
    >
      {company.logo_url ? (
        <img src={company.logo_url} alt="" className="w-9 h-9 rounded-2xl object-contain bg-background border border-border/40 flex-shrink-0" loading="lazy" />
      ) : (
      ...
      <button
        ...
        className={cn(
          "h-7 w-7 rounded-2xl flex items-center justify-center transition-colors flex-shrink-0",
                ...
                <div className="space-y-1 py-1">
                  {[...Array(4)].map((_, i) => (
                    <div key={i} className="flex items-center gap-3 px-3 py-2.5 rounded-2xl">
                      <Skeleton className="w-9 h-9 rounded-2xl opacity-20" />
