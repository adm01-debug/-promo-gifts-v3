                ...
                {(Object.entries(LASER_TONES) as [LaserTone, typeof LASER_TONES.claro][]).map(
                  ([key, tone]) => (
                    <label
                      key={key}
                      className={cn(
                        "relative flex flex-col items-center gap-2 rounded-md border-2 p-4 cursor-pointer transition-all",
                        laserTone === key
          ...
          {category === "serigrafia" && (
            <div className="space-y-4">
              ...
              {detectedColors.length > 0 ? (
                ...
                <div className="flex items-start gap-2 p-3 rounded-md bg-warning/5 dark:bg-warning/10 border border-warning/20 dark:border-warning/40">
