...
export function LogoColorAnalyzer({
  ...
  return (
    <div className={cn('space-y-3', className)}>
      ...
      {exceedsMax && (
        <div className="flex items-center gap-2 rounded-xl border border-destructive/20 bg-destructive/10 px-3 py-2">
...
function ColorRow({ color, index, onPantoneChange }: ColorRowProps) {
  return (
    <div className="grid grid-cols-2 items-center gap-2">
      {/* Detected color */}
      <div className="flex items-center gap-2 rounded-xl bg-muted/30 px-2 py-1.5">
        <div
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          className="flex w-full items-center gap-2 rounded-xl border border-border/50 px-2 py-1.5 text-left transition-colors hover:bg-muted/50"
          aria-label="Recolher"
        >
          <div
            className="h-6 w-6 shrink-0 rounded border border-border/50"
