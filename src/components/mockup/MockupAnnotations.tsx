          ...
          >
            ...
            <div className="relative">
              ...
              {ann.text && editingId !== ann.id && (
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 px-2 py-1 bg-background/95 backdrop-blur-sm border rounded-2xl shadow-md text-xs max-w-[200px] whitespace-pre-wrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                  {ann.text}
                </div>
              )}
              {/* Edit form */}
              {editingId === ann.id && (
                <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1 bg-background border rounded-2xl shadow-lg p-2 z-20 min-w-[200px]"
                  onClick={(e) => e.stopPropagation()}
                >
                  <Input
                    value={editText}
