          ...
          >
            ...
            <div className="relative">
              ...
              {ann.text && editingId !== ann.id && (
                <div className="pointer-events-none absolute bottom-full left-1/2 mb-1 max-w-[200px] -translate-x-1/2 whitespace-pre-wrap rounded-xl border bg-background/95 px-2 py-1 text-xs opacity-0 shadow-md backdrop-blur-sm transition-opacity group-hover:opacity-100">
                  {ann.text}
                </div>
              )}
              {/* Edit form */}
              {editingId === ann.id && (
                <div
                  className="absolute left-1/2 top-full z-20 mt-1 min-w-[200px] -translate-x-1/2 rounded-xl border bg-background p-2 shadow-lg"
                  onClick={(e) => e.stopPropagation()}
                >
                  <Input
                    value={editText}
