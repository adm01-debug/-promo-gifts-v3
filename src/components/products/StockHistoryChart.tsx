...
export function StockHistoryChart({ productId }: StockHistoryChartProps) {
  const {
    ...
    days,
    loadingSummary, hasData, hasError, isDemo,
    chartData, effectiveIntelligence, effectiveVelocities,
    bestVelocity, flags, supplierOptions, supplierNamesMap,
    trend, trendDisplay, marketDemandLevel, demandLabel,
    supplierText, priceChanges, turnoverScore, showTurnover,
    handleRetry,
        ...
        <div className="flex items-center justify-between flex-wrap gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            {trend != null && trend > 1.3 && (
              <Badge variant="outline" className="font-bold text-xs bg-primary/15 text-primary border-primary/30">
                🚀 Emergente
              </Badge>
            )}
            {showTurnover && (
        ...
        {flags.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mt-2" role="list" aria-label="Indicadores de mercado">
            {flags.map(flag => {
              ...
              const Icon = cfg.icon;
              return (
                <Badge key={flag} variant="outline" className={cn("gap-1 px-2 py-0.5 text-xs border", cfg.colors)} title={cfg.description} role="listitem">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2" role="group" aria-label="Métricas de inteligência de mercado">
          ...
          <KpiCard
            ...
            sub={trend != null ? (trend > 1 ? '↑ crescendo' : trend < 0.8 ? '↓ caindo' : '→ estável') : ''}
          ...
          />
          <KpiCard
            icon={trend != null && trend > 1.2 ? TrendingUp : trend != null && trend < 0.8 ? TrendingDown : BarChart3}
            label="Tendência"
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {supplierOptions.length > 1 && (
            <SupplierChartFilter suppliers={supplierOptions} selected={selectedSupplier} onSelect={setSelectedSupplier} />
          )}
        </div>
        ...
        <div className="h-[160px] sm:h-[200px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={chartData} margin={{ top: 5, right: 5, left: 0, bottom: 5 }}>
              ...
              <Legend wrapperStyle={{ fontSize: '10px', paddingTop: '4px' }} iconSize={8} formatter={(value: string) => <span className="text-muted-foreground text-[10px]">{value}</span>} />
        ...
        {effectiveVelocities.length > 1 && supplierNamesMap && (
          <SupplierComparisonCards velocities={effectiveVelocities} supplierNames={supplierNamesMap} />
        )}
        ...
        <div className="flex items-center justify-between flex-wrap gap-2">
          ...
          <Button variant="ghost" size="sm" className="h-5 text-[10px] gap-1 px-2 text-muted-foreground" onClick={() => setShowCost(!showCost)}>
