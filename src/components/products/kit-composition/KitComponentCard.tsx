      ...
      >
        ...
        <div className="shrink-0">
          <div className="relative">
            <div
              className={cn("w-16 h-16 rounded-xl flex items-center justify-center overflow-hidden border", isPackaging ? "bg-warning/5 border-warning/20" : "bg-muted/60 border-border/50", item.imageUrl && "cursor-zoom-in hover:ring-2 hover:ring-primary/40 transition-all")}
...
<img src={item.imageUrl} alt={item.productName} className="w-full h-full object-contain p-1" loading="lazy" />
      ...
      {hasSpecs && (
        <div className="px-4 pb-2.5">
          <div className="flex items-center gap-1 rounded-xl border border-border bg-muted/20 px-1 py-0.5 divide-x divide-border">
            {(item.heightMm ?? 0) > 0 && (
        ...
        <div className="px-4 pb-4 space-y-3 border-t border-border/50 animate-in slide-in-from-top-1 duration-200">
          {item.description && (
            <div className="pt-3">
              ...
              <p className="text-xs text-muted-foreground/90 whitespace-pre-line leading-relaxed bg-muted/20 rounded-xl p-3">{item.description}</p>
            </div>
          )}
