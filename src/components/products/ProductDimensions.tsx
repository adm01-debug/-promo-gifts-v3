...
function SpecItem({
  ...
  if (compact) {
    return (
      <div className="flex items-center gap-2.5 rounded-xl border border-border/40 bg-secondary/40 px-3 py-3">
        <div
          className={`h-8 w-8 rounded-xl ${iconBgClass} flex shrink-0 items-center justify-center [&_svg]:h-4 [&_svg]:w-4`}
        >
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-1.5 rounded-xl border border-border bg-secondary/50 p-4 text-center">
      <div
        className={`h-10 w-10 rounded-xl ${iconBgClass} flex shrink-0 items-center justify-center`}
      >
        <span className={iconColorClass}>{icon}</span>
      </div>
