    ...
    <Card className="border-[1.5px]">
      <CardContent className="p-5">
        ...
        <div className="relative pl-6">
          <div className="absolute left-2 top-2 bottom-2 w-px bg-gradient-to-b from-primary/40 via-border to-transparent" />
          {bi.recentOrders.map((o, i) => {
            const prev = bi.recentOrders[i + 1];
            const deltaPct = prev && prev.total > 0 ? ((o.total - prev.total) / prev.total) * 100 : null;
            const deltaTone =
              deltaPct == null
