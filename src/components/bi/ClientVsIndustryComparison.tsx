...
function MetricRow({ metric }: { metric: MetricComparison }) {
  ...
  return (
    <div className="space-y-2 p-3 rounded-2xl border bg-card/50">
...
function ShareOfWalletCard({ clientLtv, industryAvgLtv }: { clientLtv: number; industryAvgLtv: number }) {
  ...
  return (
    <button
      type="button"
      onClick={() => setOpen((o) => !o)}
      className="text-left space-y-2 p-3 rounded-2xl border bg-card/50 hover:border-primary/40 hover:shadow-sm transition-all"
      aria-expanded={open}
    >
      <div className="flex items-center justify-between gap-2">
