                  ...
                  <div className="flex gap-3">
                    {t.imageUrl ? (
                      <img
                        ...
                        alt={t.productName}
                        loading="lazy"
                        className="h-16 w-16 shrink-0 rounded-xl border bg-background object-contain"
                        onError={(e) => {
                          (e.currentTarget as HTMLImageElement).style.display = 'none';
                        }}
                      />
                    ) : (
                      <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-xl bg-muted/40">
            ...
            {enriched.filtered.map((t, i) => {
              ...
              return (
                <div
                  key={`${t.productName}-${i}`}
                  className={cn(
                    'group flex items-center gap-3 rounded-xl p-2.5 transition-colors hover:bg-muted/60',
                    t.alreadyBuys && 'opacity-90',
                  )}
                >
                  <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-xl bg-muted font-display text-xs font-bold text-muted-foreground">
                    {i + 1}
                  </div>
                  {t.imageUrl ? (
                    <div className="h-9 w-9 shrink-0 overflow-hidden rounded-xl border bg-muted/40">
                      <img
                        src={t.imageUrl}
                        alt={t.productName}
                        loading="lazy"
