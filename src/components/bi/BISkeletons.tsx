      ...
      <CardContent className="space-y-2">
        {Array.from({ length: rows }).map((_, i) => (
          <div key={i} className="flex items-center gap-3 p-2.5 rounded-2xl">
            <Skeleton className="h-7 w-7 rounded-2xl shrink-0" />
    ...
    <Card className="border-[1.5px]">
      ...
      <CardContent className="space-y-3">
        <div className="grid grid-cols-[80px_repeat(12,1fr)] gap-1">
          ...
          {Array.from({ length: 12 }).map((_, i) => (
            <Skeleton key={`c-${i}`} className="h-12 w-full rounded-2xl" />
          ))}
          <Skeleton className="h-4 w-16" />
          {Array.from({ length: 12 }).map((_, i) => (
            <Skeleton key={`s-${i}`} className="h-12 w-full rounded-2xl" />
          ))}
        </div>
  ...
  return (
    <Card className="border-[1.5px]">
      ...
      <CardContent className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {[0, 1, 2, 3, 4].map((i) => (
          <div key={i} className="p-3 rounded-2xl border space-y-2">
            <div className="flex items-center justify-between">
              <Skeleton className="h-3 w-20" />
