...
export function ChatHeader({
  ...
  return (
    <DialogHeader className="px-5 pt-4 pb-3 border-b border-border/30 flex-shrink-0 bg-gradient-to-b from-primary/[0.03] to-transparent">
        ...
        <div className="flex flex-wrap gap-1 mt-2">
          {getActiveFilterLabels(flowFilters).map(({ label, key, value }) => (
            <Badge key={`${key}-${value || label}`} variant="secondary"
              className="text-[9px] rounded-2xl px-1.5 py-0.5 gap-0.5 cursor-pointer hover:bg-destructive/10 hover:text-destructive transition-colors"
              onClick={() => {
                if (key === "price") {
      ...
      {clientName && (
        <div className="mt-2">
          <Badge variant="outline" className="text-[10px] rounded-2xl font-normal text-muted-foreground">
            Cliente: {clientName}
          </Badge>
        </div>
      )}
