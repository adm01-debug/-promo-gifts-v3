    ...
    <DialogHeader className="flex-shrink-0 border-b border-border/30 bg-gradient-to-b from-primary/[0.03] to-transparent px-5 pb-3 pt-4">
        ...
        <div className="mt-2 flex flex-wrap gap-1">
          {getActiveFilterLabels(flowFilters).map(({ label, key, value }) => (
            <Badge
              key={`${key}-${value || label}`}
              variant="secondary"
              className="cursor-pointer gap-0.5 rounded-xl px-1.5 py-0.5 text-[9px] transition-colors hover:bg-destructive/10 hover:text-destructive"
              onClick={() => {
                if (key === 'price') {
      ...
      {clientName && (
        <div className="mt-2">
          <Badge
            variant="outline"
            className="rounded-xl text-[10px] font-normal text-muted-foreground"
          >
            Cliente: {clientName}
          </Badge>
        </div>
