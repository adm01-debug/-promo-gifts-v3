      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          <Button variant="ghost" size="sm"
            className={cn("h-8 w-8 p-0 rounded-lg relative", activeFiltersCount > 0 && "text-primary bg-primary/10")}
          ...
          <Button variant="ghost" size="sm" onClick={onToggleHistory}
            className="h-8 w-8 p-0 rounded-lg" title={showHistory ? "Voltar ao chat" : "Histórico"} aria-label="Histórico">
            <History className="h-4 w-4" />
          </Button>
          <Button variant="ghost" size="sm" onClick={onNewConversation}
            className="h-8 w-8 p-0 rounded-lg" title="Nova conversa" aria-label="Nova conversa">
            <Plus className="h-4 w-4" />
          </Button>
          <Button variant="ghost" size="sm" onClick={onClose}
            className="h-8 w-8 p-0 rounded-lg" title="Fechar" aria-label="Fechar">
            <X className="h-4 w-4" />
          </Button>
        </div>
      </div>
        ...
        <div className="flex flex-wrap gap-1 mt-2">
          {getActiveFilterLabels(flowFilters).map(({ label, key, value }) => (
            <Badge key={`${key}-${value || label}`} variant="secondary"
              className="text-[9px] rounded-lg px-1.5 py-0.5 gap-0.5 cursor-pointer hover:bg-destructive/10 hover:text-destructive transition-colors"
              onClick={() => {
                if (key === "price") {
