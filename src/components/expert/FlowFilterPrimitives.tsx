  ...
  return (
    <button onClick={onToggle} className="w-full flex items-center justify-between px-1 py-2 group/sec">
      <div className="flex items-center gap-2.5">
        ...
        {count !== undefined && count > 0 && <Badge variant="secondary" className="text-[9px] px-1.5 py-0 h-4 rounded-lg bg-primary/10 text-primary border-primary/20 font-bold">{count}</Badge>}
      ...
      {searchable && items.length > 8 && (
        <div className="relative">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={placeholder}
            className="w-full h-6 pl-7 pr-2 rounded-lg text-[10px] bg-background/30 border border-border/15 placeholder:text-muted-foreground/25 focus:outline-none focus:ring-1 focus:ring-primary/20 transition-all" />
        </div>
      )}
        ...
        {visible.map((item) => {
          ...
          return (
            <button key={item} onClick={() => onToggle(item)}
              className={cn("px-2 py-0.5 rounded-lg text-[10px] font-medium border transition-all duration-150 flex items-center gap-1",
