    ...
    >
      <div className="flex items-center gap-2.5">
        ...
        {count !== undefined && count > 0 && (
          <Badge
            variant="secondary"
            className="h-4 rounded-xl border-primary/20 bg-primary/10 px-1.5 py-0 text-[9px] font-bold text-primary"
          >
            {count}
          </Badge>
        )}
      ...
      {searchable && items.length > 8 && (
        <div className="relative">
          ...
          <input
            ...
            className="h-6 w-full rounded-xl border border-border/15 bg-background/30 pl-7 pr-2 text-[10px] transition-all placeholder:text-muted-foreground/25 focus:outline-none focus:ring-1 focus:ring-primary/20"
          />
        </div>
      )}
        ...
        {visible.map((item) => {
          ...
          return (
            <button
              ...
              className={cn(
                'flex items-center gap-1 rounded-xl border px-2 py-0.5 text-[10px] font-medium transition-all duration-150',
                isSelected
