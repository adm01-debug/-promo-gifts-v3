...
export function StockAlertsIndicator({
    ...
    try {
      ...
      for (const p of allProducts) {
        ...
        if (stock > 0 && p.updated_at && p.created_at) {
          ...
          if (updated >= sevenDaysAgo && updated.getTime() - created.getTime() > 3600_000) {
  ...
  const getStockBadge = (level?: 'low' | 'critical' | 'out') => {
    switch (level) {
      ...
      case 'critical':
        return (
          <Badge className="bg-orange px-1.5 py-0 text-[10px] text-primary-foreground">
            Crítico
          </Badge>
        );
      default:
    ...
    <div>
      <Popover open={isOpen} onOpenChange={setIsOpen}>
        ...
        >
          ...
          <button
            aria-label="Fechar"
            className="absolute right-3 top-3 z-10 flex h-7 w-7 items-center justify-center rounded-xl text-muted-foreground transition-colors hover:bg-muted/60 hover:text-foreground"
            ...
            {TABS.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    'flex items-center gap-1.5 rounded-xl border px-2.5 py-1.5 text-[11px] font-medium transition-all',
                    isActive
                      ? tab.activeColor
                  ...
                  >
                    ...
                    {item.imageUrl ? (
                      <img
                        src={item.imageUrl}
                        alt=""
                        className="h-10 w-10 flex-shrink-0 rounded-xl border border-border/30 bg-background object-contain p-0.5"
                        loading="lazy"
                      />
                    ) : (
                    ...
                    <div className="flex flex-shrink-0 flex-col gap-1">
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <button
                            className="flex h-6 w-6 items-center justify-center rounded-xl text-muted-foreground opacity-0 transition-colors hover:bg-primary/10 hover:text-primary group-hover:opacity-100"
                            onClick={(e) => {
                              e.stopPropagation();
                              setIsOpen(false);
                              navigate(`/produto/${item.productId}`);
                      ...
                      <button
                        className="flex h-6 w-6 items-center justify-center rounded-xl text-muted-foreground opacity-0 transition-colors hover:bg-destructive/10 hover:text-destructive group-hover:opacity-100"
                        onClick={(e) => {
                          e.stopPropagation();
                          dismiss(item.id);
                        }}
