      ...
      <div>
        <Popover open={isOpen} onOpenChange={setIsOpen}>
          <Tooltip delayDuration={300}>
            ...
            <TooltipContent side="bottom" className="bg-primary text-primary-foreground text-[11px] px-2 py-1 border-none">
              Alerta de Estoque
            </TooltipContent>
          </Tooltip>
          ...
          >
            ...
            <button aria-label="Fechar"
              className="absolute top-3 right-3 h-7 w-7 flex items-center justify-center rounded-2xl text-muted-foreground hover:text-foreground hover:bg-muted/60 transition-colors z-10"
              ...
              {TABS.map(tab => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "flex items-center gap-1.5 px-2.5 py-1.5 rounded-2xl text-[11px] font-medium border transition-all",
                      isActive
                        ? tab.activeColor
                    ...
                    >
                      ...
                      {item.imageUrl ? (
                        <img
                          src={item.imageUrl}
                          alt=""
                          className="w-10 h-10 rounded-2xl object-contain bg-background border border-border/30 flex-shrink-0 p-0.5" loading="lazy" />
                      ) : (
                        <div className="w-10 h-10 rounded-2xl bg-muted/40 flex-shrink-0 flex items-center justify-center">
                      ...
                      <div className="flex flex-col gap-1 flex-shrink-0">
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <button
                              className="h-6 w-6 flex items-center justify-center rounded-2xl text-muted-foreground hover:text-primary hover:bg-primary/10 transition-colors opacity-0 group-hover:opacity-100"
                              onClick={(e) => {
                                e.stopPropagation();
                                setIsOpen(false);
                                navigate(`/produto/${item.productId}`);
                        ...
                        <button
                          className="h-6 w-6 flex items-center justify-center rounded-2xl text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors opacity-0 group-hover:opacity-100"
                          onClick={(e) => {
                            e.stopPropagation();
                            dismiss(item.id);
                          }}
