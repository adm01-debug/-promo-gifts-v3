...
export function StockDashboard() {
  ...
  if (isLoading) {
    ...
    return (
      <div className="space-y-5" aria-live="polite" aria-busy="true">
        <div className="flex items-center gap-4 rounded-2xl border border-primary/20 bg-primary/[0.03] p-5 shadow-sm shadow-primary/5 duration-500 animate-in fade-in zoom-in-95">
          <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-2xl bg-primary/10 shadow-inner">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center justify-between gap-3">
              <div className="space-y-0.5">
                <p className="font-display text-sm font-bold uppercase tracking-tight text-foreground">
                  Sincronizando Estoque Realtime
                </p>
              ...
              {loadingProgress && (
                <div className="text-right">
                  <p className="text-lg font-extrabold tabular-nums leading-none text-primary">
                    {pct}%
                  </p>
                  <p className="mt-0.5 text-[9px] font-bold uppercase tracking-tighter text-muted-foreground/60">
                    Processando
                  </p>
                </div>
              )}
            ...
            <div className="mt-3">
              <div className="h-1.5 overflow-hidden rounded-full bg-primary/10 shadow-inner">
                <div
                  className="h-full rounded-full bg-gradient-to-r from-primary via-primary-glow to-primary shadow-lg transition-all duration-700 ease-out"
                  style={{ width: `${pct || 8}%` }}
                />
              </div>
            </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col items-start justify-between gap-4 rounded-2xl border border-border/40 bg-card/60 p-4 shadow-sm sm:flex-row sm:items-center">
          ...
          <div>
            ...
            <div className="mt-0.5 flex items-center gap-2">
                ...
                <Tooltip>
                  <TooltipTrigger>
                    <Badge
                      variant="outline"
                      className={cn(
                        'h-6 gap-1 px-2 text-[11px] font-bold uppercase tracking-wider shadow-sm',
                        healthScore >= 80 &&
                        ...
                        healthScore < 50 &&
                          'animate-pulse border-destructive/30 bg-destructive/10 text-destructive shadow-destructive/10',
                      )}
                    >
                  ...
                  <TooltipContent className="border-none bg-primary px-2 py-1 text-[11px] font-medium text-primary-foreground shadow-xl">
                    <p className="max-w-[220px]">
                      Disponibilidade Total: {summary.productsInStock} de {summary.totalProducts}{' '}
              ...
              {criticalAlerts.length > 0 && (
                <Badge
                  variant="destructive"
                  className="h-6 cursor-pointer gap-1.5 px-2 text-[10px] font-bold uppercase transition-transform hover:scale-105"
        ...
        <div className="flex flex-col gap-1 sm:items-end">
          <div className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-widest text-muted-foreground/60">
            ...
            Sincronizado em{' '}
            {lastRefreshRef.current.toLocaleTimeString('pt-BR', {
              hour: '2-digit',
              minute: '2-digit',
            })}
          </div>
          <div className="flex items-center gap-2">
            {isFetching && (
              <Badge
                variant="outline"
                className="h-5 gap-1 border-primary/30 bg-primary/5 px-1.5 text-[10px] text-primary animate-in fade-in slide-in-from-right-1"
              >
