...
export function AIRecommendationsPanel({
  ...
  const handleGenerate = useCallback(() => {
    if (!client.name.trim() || products.length === 0) return;
    fetchRecommendations(client, products);
  }, [client, products, fetchRecommendations]);

  const canGenerate = client.name.trim().length > 0 && products.length > 0 && !isLoading;

  return (
    <div className={cn('space-y-4 font-display', className)}>
      {/* Formulário do cliente */}
      {!hideClientForm && (
        <Card className="rounded-xl border-[1.5px] border-border">
      ...
      {!isLoading && recommendations.length > 0 && (
        <div className="animate-fade-in space-y-4">
          ...
          {insights && (
            <Card className="rounded-xl border-[1.5px] border-primary/30 bg-primary/5">
