      ...
      {logos.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {logos.map((logo) => {
            ...
            return (
              <div key={logo.id} className={cn("rounded-2xl border bg-card p-2 space-y-2", active ? "border-primary/40 ring-1 ring-primary/20" : "border-border")}>
                <button type="button" className="flex w-full items-center gap-2 text-left" onClick={() => onUseLogo(logo)} aria-label={`Aplicar ${logo.label}`}>
                  <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border bg-background p-1">
