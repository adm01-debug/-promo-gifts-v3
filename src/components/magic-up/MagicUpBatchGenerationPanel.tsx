...
export function MagicUpBatchGenerationPanel({
  queue,
  running,
  canGenerate,
  onSetQueue,
  onRunQueue,
  onClearQueue,
}: MagicUpBatchGenerationPanelProps) {
  return (
    <Card>
      <CardHeader className="pb-3">
        ...
        <CardDescription className="text-xs">
          Monte uma fila local de variações para canais, cenas e tons.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-3">
        <div className="grid grid-cols-2 gap-2">
          {BATCH_PRESETS.map((preset) => {
            const active =
              queue.length === preset.variants.length &&
              preset.variants.every((variant, index) => queue[index]?.id === variant.id);
            return (
              <button
                key={preset.id}
                type="button"
                onClick={() => onSetQueue(preset.variants)}
                className={cn(
                  'rounded-xl border px-2.5 py-2 text-left text-xs font-medium transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring',
                  active
          ...
          <div className="space-y-2 rounded-xl border bg-muted/30 p-2.5">
            <div className="flex items-center justify-between gap-2">
              <Badge variant="outline" className="text-[10px]">
                {queue.length} variações na fila
              </Badge>
              <Button
                type="button"
                variant="ghost"
        ...
        <Button
          ...
          disabled={!queue.length || !canGenerate || running}
          onClick={onRunQueue}
        >
