      ...
      <CardContent className="space-y-3">
        <div className="grid grid-cols-2 gap-2">
          {BATCH_PRESETS.map((preset) => {
            ...
            return (
              <button key={preset.id} type="button" onClick={() => onSetQueue(preset.variants)} className={cn("rounded-xl border px-2.5 py-2 text-left text-xs font-medium transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring", active ? "border-primary bg-primary/10 text-primary" : "border-border bg-muted/30 text-foreground hover:border-primary/50")} aria-pressed={active}>
                {preset.label}
              </button>
            );
          })}
