    ...
    <section className="rounded-lg border bg-muted/30 p-3" aria-label="Checklist de curadoria">
      ...
      <div className="mt-2 space-y-2">
        {diagnosis.criteria.map((criterion) => (
          <div key={criterion.id} className="flex items-start gap-2 rounded-md bg-background/60 p-2">
            {criterion.passed ? <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-primary" /> : <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-destructive" />}
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between gap-2">
                <span className="text-xs font-medium text-foreground">{criterion.label}</span>
                <span className={cn("text-[11px] font-semibold", criterion.passed ? "text-primary" : "text-destructive")}>{criterion.score}</span>
              </div>
