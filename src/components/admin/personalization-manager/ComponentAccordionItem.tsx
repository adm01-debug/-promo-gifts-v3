                          ...
                          <Tooltip key={lt.id}>
                            <TooltipTrigger asChild>
                              <Badge variant={lt.is_default ? "default" : "outline"} className="text-xs gap-1 group cursor-pointer" onClick={() => updateTechnique({ id: lt.id, is_default: !lt.is_default })}>
                                ...
                                <button className="opacity-0 group-hover:opacity-100 transition-opacity ml-1" onClick={(e) = aria-label="Fechar"> { e.stopPropagation(); deleteTechnique(lt.id); }}><X className="h-2 w-2" /></button>
                              </Badge>
                            </TooltipTrigger>
