...
export function FailureWindowCard() {
  ...
  async function save() {
    ...
    setSaving(true);
    const { data, error } = await supabase.rpc("set_connection_failure_window_minutes", { minutes });
    setSaving(false);
    if (error) {
    ...
    <Card className="border-dashed">
      <CardContent className="flex flex-wrap items-center gap-3 py-3">
        <div className="flex items-center gap-2">
          <div className="h-8 w-8 rounded-2xl bg-warning/10 flex items-center justify-center">
        ...
        <TooltipProvider delayDuration={200}>
          <Tooltip>
            ...
            <TooltipContent side="top" className="max-w-[300px]">
              <p className="text-xs">
                Evita ruído de incidentes em flaps transitórios. O cron de health-check
                só dispara <code>connection_down</code> se nenhum teste sucesso ocorreu
                dentro da janela. Alterações são auditadas.
              </p>
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
