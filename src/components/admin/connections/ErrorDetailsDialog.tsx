...
function Section({ title, defaultOpen = true, children }: SectionProps) {
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen} className="rounded-2xl border bg-card">
      ...
      <CollapsibleContent className="px-3 pb-3 pt-1 text-xs space-y-2">
        {children}
      </CollapsibleContent>
    </Collapsible>
  );
...
function KV({ k, v }: { k: string; v: React.ReactNode }) {
  return (
    <div className="grid grid-cols-[120px_1fr] gap-2">
      ...
      <span className="font-mono break-all">{v ?? <em className="text-muted-foreground">—</em>}</span>
    </div>
  );
}
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          ...
          <div className="flex items-start justify-between gap-3 pr-6">
            <div className="min-w-0">
              <DialogTitle className="flex items-center gap-2 flex-wrap">
                <Badge variant="outline" className="border-destructive/40 text-destructive text-[10px] uppercase tracking-wide font-semibold">
                  {error.categoryLabel}
                </Badge>
        ...
        <div className="space-y-3">
          ...
          {!loading && !details && (
            <div className="rounded-2xl border border-dashed bg-muted/30 px-3 py-6 text-center text-xs text-muted-foreground">
          ...
          {!loading && details && (
            <>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {[
                      ...
                      <div key={label as string} className="rounded border bg-muted/30 px-2 py-1.5">
                        ...
                        <div className="font-mono text-sm">{fmtMs(value as number | null | undefined)}</div>
                      </div>
                    ))}
                  </div>
                </Section>
              ...
              <Section title="Resposta crua" defaultOpen={false}>
                ...
                <div className="space-y-1">
                  <div className="text-[10px] uppercase tracking-wide text-muted-foreground">
                    Body {details.response.truncated && <span className="text-warning">(truncado)</span>}
                  </div>
                  {details.response.body ? (
                    <pre className="rounded border bg-muted/30 p-2 max-h-64 overflow-auto whitespace-pre-wrap break-all font-mono text-[11px]">
                      {details.response.body}
                    </pre>
                  ) : (
