 ...
 */
import { Activity, AlertTriangle, AlertOctagon, CheckCircle2, RefreshCw, Webhook, Clock, XCircle } from "lucide-react";
...
const SEVERITY_META: Record<
  PulseSeverity,
  { label: string; tone: string; icon: typeof CheckCircle2; pulseColor: string; description: string }
> = {
  P0: {
    label: "P0 · Crítico",
    ...
    description: "Severidade P0 (crítico): impacto imediato no fluxo de integrações — exige intervenção agora",
  },
  P1: {
    label: "P1 · Atenção",
...
function MiniKpi({
  ...
  const content = (
    <div
      className={cn(
        "flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-background/60 border min-w-0 transition-colors",
...
export function ConnectionsPulseBar() {
  ...
  const successTone =
    successRate === null ? "default" : successRate >= 95 ? "success" : successRate >= 70 ? "warning" : "destructive";
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <div className="flex items-center gap-2.5 min-w-0">
                <span className="relative flex h-2.5 w-2.5 shrink-0" aria-hidden="true">
                  ...
                  <span className={cn("relative inline-flex rounded-full h-2.5 w-2.5", meta.pulseColor)} />
                </span>
                <Badge variant="outline" className={cn("gap-1 font-semibold text-[11px]", meta.tone)}>
            ...
            <TooltipContent side="bottom" className="max-w-sm">
              <p className="text-xs font-semibold mb-1">{meta.description}</p>
              {data && data.reasons.length > 0 ? (
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            <MiniKpi
              ...
              value={
                data?.kpis.lastSuccessAt
                  ? formatDistanceToNow(new Date(data.kpis.lastSuccessAt), { locale: ptBR, addSuffix: true })
                  : "—"
              }
            ...
            <button
              type="button"
              onClick={() => refetch()}
              className="ml-1 inline-flex items-center gap-1 rounded-lg border border-border/50 bg-background/60 px-2 py-1 text-[11px] text-muted-foreground hover:text-foreground hover:bg-muted/60 transition-colors"
