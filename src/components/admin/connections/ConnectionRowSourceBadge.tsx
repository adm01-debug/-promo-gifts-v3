...
export function ConnectionRowSourceBadge({
  ...
  if (!envKey) {
    ...
    return (
      <span
        className={cn(
          "inline-flex items-center gap-1 rounded-2xl border px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide",
          m.cls,
          className,
        )}
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <span
          tabIndex={0}
          className={cn(
            "inline-flex items-center gap-1 rounded-2xl border px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide cursor-help focus:outline-none focus:ring-2 focus:ring-ring",
            m.cls,
            className,
          )}
