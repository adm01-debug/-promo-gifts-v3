    ...
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl">
          ...
          {summary && (
            <div className="flex items-center flex-wrap gap-2 rounded-2xl border bg-muted/20 px-3 py-2 text-xs">
              {summary.ok > 0 && (
                <Badge
                  variant="outline"
...
function ResultRow({ result }: { result: PerEnvResult }) {
  ...
  return (
    <li className={`rounded-2xl border p-3 ${headerTone}`}>
...
function TestDetails({ test }: { test: TestResult }) {
  ...
  return (
    <div className="mt-2 rounded-2xl border border-destructive/40 bg-destructive/5 p-2 text-xs">
