 ...
 */
import { Activity, Settings2, Network, Eye, EyeOff, RotateCcw } from "lucide-react";
  ...
  const handleAnchorClick = (e: React.MouseEvent, zone: ZoneDef) => {
    ...
    if (!visible[zone.id]) {
      ...
      requestAnimationFrame(() => {
        document.getElementById(`zone-${zone.id}`)?.scrollIntoView({ behavior: "smooth", block: "start" });
      });
    }
  };
      ...
      >
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <a
                    ...
                    className={cn(
                      "inline-flex items-center gap-1.5 pl-2.5 pr-2 py-1 transition-colors",
                      isVisible
                        ? "text-muted-foreground hover:text-foreground hover:bg-muted/60"
                        : "text-muted-foreground/70 hover:text-foreground line-through decoration-dotted",
                    )}
                ...
                <TooltipContent side="bottom" className="max-w-[220px]">
                  <p className="text-xs">
                    {isVisible ? "Clique para rolar até a zona." : "Zona oculta — clique para mostrar e rolar."}
                  </p>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <button
                    ...
                    className={cn(
                      "inline-flex items-center justify-center px-1.5 border-l transition-colors",
                      isVisible
                        ? "border-border/60 text-muted-foreground hover:text-foreground hover:bg-muted/60"
                        : "border-border/40 text-muted-foreground/70 hover:text-foreground hover:bg-muted/60",
                    )}
                  >
        ...
        {hiddenCount > 0 && (
          <button
            type="button"
            onClick={onShowAll}
            className="inline-flex items-center gap-1 rounded-full border border-primary/40 bg-primary/10 px-2.5 py-1 text-primary hover:bg-primary/15 transition-colors"
            aria-label="Mostrar todas as zonas"
          >
            <RotateCcw className="h-3 w-3" />
            <span>Mostrar tudo ({hiddenCount} oculta{hiddenCount === 1 ? "" : "s"})</span>
          </button>
        )}
      </nav>
    </TooltipProvider>
