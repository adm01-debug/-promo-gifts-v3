...
export function ProductFiscalSection({ register, numericProps }: Props) {
  return (
    <SectionCard id="fiscal" title="Dados Fiscais do Produto" icon={FileText} subtitle="Atributos fiscais intrínsecos ao produto — não variam por fornecedor">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <FieldLabel htmlFor="ipi_rate" hint="Imposto sobre Produtos Industrializados — alíquota padrão definida pelo NCM">Alíquota IPI (%)</FieldLabel>
        ...
        <div>
          ...
          <select id="country_of_origin" {...register('country_of_origin')} className="h-9 w-full rounded-2xl border border-input bg-background px-3 text-sm">
      ...
      <p className="text-xs text-muted-foreground mt-1">
        Campos fiscais que variam por fornecedor (CFOP, CST, ICMS, PIS, COFINS, CEST) são gerenciados na aba <strong>Identificação → Fontes de Fornecimento</strong>.
      </p>
    </SectionCard>
  );
}
