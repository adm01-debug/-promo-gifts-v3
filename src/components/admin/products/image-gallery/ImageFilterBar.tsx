      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="flex items-center gap-1 p-0.5 rounded-lg bg-muted/50 border border-border/40">
          <button type="button" onClick={() => setFilterMode('all')}
            className={cn("px-2.5 py-1 rounded-lg text-[11px] font-medium transition-all", filterMode === 'all' ? "bg-background shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground")}>
            Todas
          </button>
          <button type="button" onClick={() => setFilterMode('general')}
            className={cn("px-2.5 py-1 rounded-lg text-[11px] font-medium transition-all", filterMode === 'general' ? "bg-background shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground")}>
            Gerais ({stats.withoutVariant})
          </button>
          {hasVariants && (
            <button type="button" onClick={() => setFilterMode('by-variant')}
              className={cn("px-2.5 py-1 rounded-lg text-[11px] font-medium transition-all flex items-center gap-1", filterMode === 'by-variant' ? "bg-background shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground")}>
