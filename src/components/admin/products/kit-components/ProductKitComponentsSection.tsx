...
import type { KitComponent, ComponentFormData, BoxInternalDimensions } from './types';
          ...
          {components.map((comp) => {
            ...
            return (
              <div key={comp.id} className="space-y-0.5">
                <div className={cn('flex items-center gap-2.5 rounded-2xl border p-2.5 transition-colors group', 'hover:bg-accent/50')}>
                  {comp.primary_image_url ? (
                    
<img src={comp.primary_image_url} alt={comp.component_name || ''} className="w-10 h-10 rounded-2xl object-contain border shrink-0 bg-muted/30"  loading="lazy" />
                  ) : (
                    <div className="w-10 h-10 rounded-2xl border shrink-0 bg-muted flex items-center justify-center">
