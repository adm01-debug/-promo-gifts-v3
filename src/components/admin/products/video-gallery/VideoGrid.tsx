...
import {
  Film, Play, Link2, Trash2, Unlink, Loader2, ImagePlus, GripVertical, Type,
} from 'lucide-react';
import type { ExternalVideo, VariantLink, VideoVariant } from './types';
import { VIDEO_TYPES, formatBytes } from './types';
...
interface Props {
  ...
  updateVideoMeta: (videoId: string, data: { title?: string; description?: string; video_type?: string }) => void;
...
export function VideoGrid({
  filteredVideos, videoLinksMap, variantMap, variants,
  linkingVideoId, setLinkingVideoId, editingVideoId, setEditingVideoId,
  regeneratingId, dragIndex, dragOverIndex,
  getThumbnail, setPreviewVideo, linkVideoToVariant, unlinkVideoFromVariant,
  regenerateThumbnail, requestRemove, updateVideoMeta,
      ...
      {filteredVideos.map((video, index) => {
        ...
        return (
          <div
            ...
            className={cn(
              'relative group rounded-2xl border-2 overflow-hidden aspect-video transition-all cursor-pointer',
...
<img src={thumbnail} alt={video.title || 'Vídeo'} className="w-full h-full object-cover" loading="lazy" />
            ...
            <div className="absolute top-1.5 left-1.5 flex flex-col gap-0.5">
              {video.is_primary && <Badge className="text-[9px] px-1 py-0 bg-primary text-primary-foreground">Principal</Badge>}
              {typeInfo && (
                <Badge variant="outline" className="text-[8px] px-1 py-0 bg-background/80 backdrop-blur-sm">{typeInfo.label}</Badge>
              )}
            </div>
                ...
                {links.map(link => (
                  <Tooltip key={link.id}>
                    <TooltipTrigger asChild>
                      <span className="w-3.5 h-3.5 rounded-full border border-white/60 shadow-sm" style={{ backgroundColor: link.variant_color_hex || '#999' }} />
                    </TooltipTrigger>
                    <TooltipContent className="text-[10px]">{link.variant_name || 'Variação'}</TooltipContent>
                  </Tooltip>
                ))}
              </div>
            )}
