...
<img src={editingSupplier.logo_url} alt="Logo" className="w-full h-full object-contain"  loading="lazy" />
                  ...
                  <button type="button" onClick={() => logoInputRef.current?.click()} disabled={uploadingLogo} className="w-20 h-20 rounded-md border-2 border-dashed border-border hover:border-primary/50 flex flex-col items-center justify-center gap-1 text-muted-foreground hover:text-primary transition-colors">
            ...
            <div className="flex items-center justify-between rounded-md border border-border p-3"><Label className="text-sm">Ativo</Label><Switch checked={editingSupplier.active ?? true} onCheckedChange={v => updateField('active', v)} /></div>
          </TabsContent>
          ...
          <TabsContent value="contact" className="space-y-3 pt-3">
            ...
            <div className="space-y-3 max-h-[320px] overflow-y-auto pr-1">
              {contacts.map((contact, index) => (
                <div key={contact.id} className="rounded-md border border-border bg-muted/30 p-3 space-y-3">
          ...
          <TabsContent value="financial" className="space-y-4 pt-3">
            ...
            {formaPagamento.includes('PIX') && (
              <div className="rounded-md border border-border bg-muted/30 p-4 space-y-4">
                ...
                {pixKeys.map((pix, idx) => (
                  <div key={pix.id} className="rounded-md border border-border bg-background/50 p-3 space-y-3">
                    <div className="flex items-center justify-between">
