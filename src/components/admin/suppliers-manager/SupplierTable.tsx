...
<img src={supplier.logo_url} alt="Logo do fornecedor" className="w-8 h-8 rounded object-contain border border-border bg-muted shrink-0"  loading="lazy" />
                  ...
                  <TableCell>
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Button variant="ghost" size="icon" className="h-7 w-7" onClick={(e) => { e.stopPropagation(); onEdit(supplier); }}><Pencil className="h-3.5 w-3.5" /></Button>
                      <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:text-destructive" onClick={(e) => { e.stopPropagation(); onDelete(supplier); }} disabled={deleting === supplier.id}>
