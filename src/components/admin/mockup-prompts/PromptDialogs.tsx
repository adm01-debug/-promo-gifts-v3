...
interface HistoryDialogProps {
  ...
  loading: boolean;
  onClose: () => void;
  onRestore: (entry: PromptHistory) => void;
}
      ...
      <DialogContent className="max-w-2xl">
        ...
        <ScrollArea className="max-h-[60vh]">
          {loading ? (
            ...
            <p className="py-8 text-center text-muted-foreground">Nenhum histórico ainda</p>
          ) : (
              ...
              {history.map((entry) => (
                <Card key={entry.id} className="border-border/50">
                  <CardHeader className="pb-2">
                    ...
                    {entry.change_notes && (
                      <p className="text-xs italic text-muted-foreground">{entry.change_notes}</p>
                    )}
                  </CardHeader>
                  <CardContent>
                    <div className="mb-1 text-xs text-muted-foreground">
                      Modelo: {entry.ai_model}
                    </div>
                    <pre className="max-h-40 overflow-auto whitespace-pre-wrap rounded-xl bg-muted/50 p-3 text-xs">
                      {entry.prompt_text}
                    </pre>
                  </CardContent>
                </Card>
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-2xl">
        ...
        {loading ? (
          ...
          <ScrollArea className="max-h-[60vh]">
            <pre className="whitespace-pre-wrap rounded-xl bg-muted/50 p-4 text-sm">{result}</pre>
          </ScrollArea>
        ) : null}
      </DialogContent>
    </Dialog>
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Crie um prompt customizado para uma técnica específica
          </DialogDescription>
        </DialogHeader>
        <div className="py-4">
          <Label>Técnica</Label>
        ...
        <DialogFooter>
          ...
          <Button onClick={onAdd} disabled={!selected}>
