              ...
              {history.map((entry) => (
                <Card key={entry.id} className="border-border/50">
                  ...
                  <CardContent>
                    ...
                    <pre className="text-xs bg-muted/50 p-3 rounded-2xl whitespace-pre-wrap max-h-40 overflow-auto">{entry.prompt_text}</pre>
                  </CardContent>
                </Card>
              ))}
            </div>
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-2xl">
        ...
        {loading ? <div className="flex justify-center py-8"><Loader2 className="h-6 w-6 animate-spin" /></div>
          : result ? <ScrollArea className="max-h-[60vh]"><pre className="text-sm bg-muted/50 p-4 rounded-2xl whitespace-pre-wrap">{result}</pre></ScrollArea> : null}
      </DialogContent>
    </Dialog>
  );
}
