                    ...
                    {filteredProducts.map((product) => {
                      ...
                      return (
                        <button key={product.id} onClick={() => onProductClick(product)} className={cn(
                          "group w-full flex items-center gap-3 px-3 py-2.5 rounded-md border transition-all text-left",
...
<img loading="lazy" src={`${product.images[0]}/thumbnail`} alt={product.name} className="h-11 w-11 object-cover rounded-md bg-muted"
                                onError={(e) => { const t = e.currentTarget; if (t.src.includes('/thumbnail')) t.src = product.images![0]; else t.style.display = 'none'; }} />
                            ) : (
                              <div className="h-11 w-11 bg-muted rounded-md flex items-center justify-center"><Package className="h-5 w-5 text-muted-foreground" /></div>
                            )}
