...
export function QuotesConfigurableList({
  ...
  const visibleIds = useMemo(
    () => paginatedQuotes.map((q) => q.id!).filter(Boolean),
    [paginatedQuotes],
  );
          ...
          {paginatedQuotes.map((quote) => (
            <TableRow
              ...
              className={cn(
                ...
                (isSelected(quote.id!) || allPagesSelected) &&
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild onClick={(e) => e.stopPropagation()}>
                      <Button
                        variant="ghost"
                        size="icon"
                        className="h-8 w-8 transition-all duration-300 hover:bg-primary/20 hover:text-primary"
                      >
