  ...
  return (
    <div ref={ref} className="relative">
      <button
        type="button"
        className={cn(
          'flex h-10 w-full items-center justify-between rounded-xl border border-input bg-background px-3 py-2 text-sm ring-offset-background transition-colors hover:bg-accent/50 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2',
        )}
      ...
      {open && (
        <div className="absolute z-50 mt-1 max-h-[280px] w-full overflow-y-auto rounded-xl border border-border bg-popover shadow-lg">
          {contacts.map((contact) => (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                {contact.cargo && (
                  <p className="text-[11px] text-muted-foreground">{contact.cargo}</p>
                )}
              </div>
            </button>
          ))}
