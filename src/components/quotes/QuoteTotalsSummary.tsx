      ...
      <div className="w-full max-w-sm overflow-hidden rounded-xl border border-border">
        <div className="space-y-2 p-4">
          <div className="flex justify-between text-sm">
            ...
            <span data-testid="summary-subtotal-products">
              {formatCurrency(productSubtotal * markupMultiplier)}
            </span>
          </div>
          {hasPersonalizations && (
            <div className="flex justify-between text-sm">
          ...
          {shippingType && (
            <div className="flex justify-between text-sm">
              ...
              <span>
                {shippingType === 'cif'
                  ? 'CIF — Cortesia'
                  : shippingType === 'fob' && !shippingCost
                    ? 'FOB — Por conta do cliente'
                    : shippingType === 'fob' || shippingType === 'fob_pre'
                      ? `FOB — Repassado ao cliente (${formatCurrency(shippingCost || 0)})`
        ...
        <div className="border-t border-border bg-muted/50 px-4 py-3">
          <div className="flex items-baseline justify-between">
            ...
            <span data-testid="summary-total" className="text-xl font-bold text-primary">
