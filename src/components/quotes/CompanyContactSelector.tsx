  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        {!companyId ? (
          <div className={cn("flex h-10 w-full items-center rounded-lg border border-input bg-background px-3 py-2 text-sm text-muted-foreground")}>Selecione uma empresa primeiro</div>
        ) : loadingContacts ? (
          <div className={cn("flex h-10 w-full items-center gap-2 rounded-lg border border-input bg-background px-3 py-2 text-sm text-muted-foreground")}><Loader2 className="h-4 w-4 animate-spin" />Carregando...</div>
        ) : !contacts || contacts.length === 0 ? (
          <div className={cn("flex h-10 w-full items-center rounded-lg border border-input bg-background px-3 py-2 text-sm text-muted-foreground")}>Nenhum contato cadastrado</div>
        ) : contacts.length === 1 ? (
