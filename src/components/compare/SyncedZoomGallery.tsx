...
export function SyncedZoomGallery({ products, onProductClick }: SyncedZoomGalleryProps) {
  ...
  const GalleryContent = ({ inDialog = false }: { inDialog?: boolean }) => (
    <div className={cn('space-y-4', inDialog && 'p-4')}>
      <div className="flex items-center justify-center gap-4 rounded-xl border border-border/30 bg-muted/50 p-3 backdrop-blur-md">
        <Button
          variant="ghost"
          size="icon"
                  ...
                  {colors.slice(0, 8).map((c, idx) => (
                    <button
                      ...
                      className={cn(
                        ...
                        colorIdx === idx
                          ? 'scale-110 border-amber-500 shadow-md ring-2 ring-amber-500/30'
                ...
                <div className="flex justify-center gap-1.5 overflow-x-auto pb-1">
                  {product.images.slice(0, 5).map((img, idx) => (
                    <button
                      ...
                      className={cn(
                        'h-10 w-10 shrink-0 overflow-hidden rounded-xl transition-all',
                        colorIdx === idx
