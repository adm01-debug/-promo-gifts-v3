      ...
      <AnimatePresence>
        {headerStuck && (
          <motion.div
            ...
            exit={{ y: -40, opacity: 0 }}
            className="sticky top-[calc(var(--header-h,56px)+var(--breadcrumb-h,0px))] z-30 border-b border-border bg-background/95 px-2 py-2 shadow-sm backdrop-blur-md"
          >
            <div className="flex items-center gap-2 overflow-x-auto">
              {entries.map((entry) => (
                <div
                  key={`sticky-${entry.index}`}
                  className="flex min-w-[180px] shrink-0 items-center gap-2 rounded-xl border border-border bg-card px-2 py-1"
                >
                  <img
                      ...
                      <div className="group relative" data-compare-product={entry.index}>
                        ...
                        <div className="flex flex-col items-center gap-2">
                          <img
                            src={hoveredVariant[entry.index] ?? entry.product.images[0]}
                            alt={entry.product.name}
                            className="h-24 w-24 cursor-pointer rounded-xl object-cover transition-all hover:ring-2 hover:ring-primary"
                          ...
                          {(entry.product.colors?.length ?? 0) > 1 && (
                            <div className="flex flex-wrap justify-center gap-0.5">
                              {entry.product.colors.slice(0, 6).map((c: any, i: number) => (
                                <button
                                  key={i}
                                  type="button"
                                  ...
                                  onMouseLeave={() =>
                                    setHoveredVariant((prev) => ({ ...prev, [entry.index]: null }))
                                  }
                                />
                              ))}
                            </div>
                          ...
                          {scores.find((s) => String(s.productId) === String(entry.product.id))
                            ?.isWinner && (
                            <Badge className="mt-1 animate-pulse gap-1 border-none bg-amber-500 px-1.5 py-0.5 text-[9px] text-white">
              ...
              <SimpleRow
                ...
                render={(p) => (
                  <div className="flex flex-wrap justify-center gap-1">
                    {p.colors?.slice(0, 6).map((c: any, i: number) => (
                      <div
                        key={i}
