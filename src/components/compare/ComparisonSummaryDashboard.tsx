...
export function ComparisonSummaryDashboard({ products }: ComparisonSummaryDashboardProps) {
  ...
  const winnerProduct = winnerScore
    ? products.find((p) => String(p.id) === winnerScore.productId)
    : null;
  ...
  const bestVariety = [...products].sort(
    (a, b) => (b.colors?.length || 0) - (a.colors?.length || 0),
  )[0];
  ...
  return (
    <div className="grid grid-cols-1 gap-4 duration-500 animate-in fade-in slide-in-from-top-4 md:grid-cols-3">
      {/* Coluna 1: Resultado Geral */}
      <div className="flex flex-col justify-between rounded-xl border-[2px] border-amber-400/20 bg-gradient-to-br from-amber-400/5 to-transparent p-4 shadow-sm">
        <div className="mb-3 flex items-center justify-between">
          <h3 className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-amber-600">
            <TrendingUp className="h-4 w-4" />
            Veredito Global
          </h3>
          <Badge
            variant="secondary"
            className="border-amber-500/20 bg-amber-500/10 text-[9px] font-black uppercase text-amber-700"
          >
            Ponderado
          </Badge>
        </div>
        ...
        {winnerProduct && (
          <div className="space-y-2">
            <div className="flex items-end gap-2">
              ...
              <span className="mb-1 text-xs font-bold uppercase tracking-tighter text-muted-foreground">
                Índice Global
              </span>
            </div>
            <p className="line-clamp-2 text-sm font-bold text-foreground/80">
              {winnerProduct.name}
            </p>
            <div className="flex items-center gap-1.5 rounded-lg border border-amber-500/20 bg-amber-500/10 p-1.5 text-[10px] font-black uppercase tracking-widest text-amber-600">
      ...
      <div className="rounded-xl border-[2px] border-blue-400/20 bg-gradient-to-br from-blue-400/5 to-transparent p-4 shadow-sm">
        <div className="mb-3 flex items-center justify-between">
          <h3 className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-blue-600">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between text-[11px]">
            ...
            <span className="max-w-[120px] truncate font-bold text-blue-700" title={bestPrice.name}>
              {bestPrice.name}
            </span>
          </div>
          <div className="flex items-center justify-between text-[11px]">
            ...
            <span
              className="max-w-[120px] truncate font-bold text-green-700"
              title={bestStock.name}
            >
              {bestStock.name}
            </span>
          </div>
          <div className="flex items-center justify-between text-[11px]">
            ...
            <span
              className="max-w-[120px] truncate font-bold text-purple-700"
              title={bestVariety.name}
            >
              {bestVariety.name}
            </span>
          </div>
      ...
      <div className="flex flex-col justify-between rounded-xl border-[2px] border-destructive/20 bg-gradient-to-br from-destructive/5 to-transparent p-4 shadow-sm">
        <div className="mb-3 flex items-center justify-between">
        ...
        <div className="space-y-2">
          <div className="text-xs font-medium">
            {products.length === 2 ? (
              <span className="font-bold uppercase tracking-wider text-destructive">
                Duelo 1v1 Disponível
              </span>
            ) : (
              <span className="uppercase tracking-wider text-muted-foreground">
                Comparação de Grupo ({products.length} itens)
              </span>
            )}
          </div>
          ...
          <div
            className={cn(
              'mt-2 rounded p-1 text-center font-mono text-[10px]',
              products.length > 4 ? 'bg-amber-100 text-amber-700' : 'bg-muted',
            )}
          >
            {products.length > 4
              ? 'Atenção: Volume alto de dados no mobile'
