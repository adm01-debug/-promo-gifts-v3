  ...
  return (
    <div className={className}>
      <div className="rounded-xl border-[2px] border-amber-400/20 bg-gradient-to-br from-amber-400/5 to-transparent p-4 shadow-sm">
        <h3 className="mb-4 flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-amber-600">
          <span className="inline-block h-4 w-1.5 animate-pulse rounded-full bg-amber-500" />
          ...
          <RadarChart data={data} outerRadius="75%">
            ...
            {products.map((p, i) => (
              <Radar
                ...
                fillOpacity={
                  opacity[String(p.id)] ?? (Object.keys(opacity).length > 0 ? 0.05 : 0.18)
                }
                strokeOpacity={
                  opacity[String(p.id)] ? 1 : Object.keys(opacity).length > 0 ? 0.2 : 0.8
                }
                strokeWidth={2}
