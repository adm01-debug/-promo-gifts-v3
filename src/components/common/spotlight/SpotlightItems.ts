...
export function buildSpotlightItems(navigate: (path: string) => void): SpotlightItem[] {
  return [
    ...
    { id: "kit-library", title: "Biblioteca de Kits", description: "Meus kits + sugeridos (G K)", icon: React.createElement(Package, { className: "h-4 w-4" }), action: () => navigate("/meus-kits"), category: "Navegação", shortcut: "G K" },
    { id: "kit-builder", title: "Montar novo kit", description: "Abrir Kit Maker", icon: React.createElement(Plus, { className: "h-4 w-4" }), action: () => navigate("/montar-kit"), category: "Navegação" },
