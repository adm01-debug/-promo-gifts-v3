...
export function SearchResultItem({ result, index, selectedIndex, query, onSelect, onHover }: ResultItemProps) {
  ...
  return (
    <motion.button
      ...
      className={cn(
        "w-full flex items-center gap-3 px-3 py-2 rounded-2xl text-left transition-all duration-150 group",
        selectedIndex === index
    ...
    >
      <div className={cn(
        "flex items-center justify-center w-10 h-10 rounded-2xl flex-shrink-0 transition-all duration-200 overflow-hidden border border-transparent",
