  ...
  const performSemanticSearch = useCallback(async (searchQuery: string) => {
    ...
    try {
      ...
      if (
        intent.type === 'order' ||
        intent.type === 'mixed' ||
        intent.entities?.includes('order')
      ) {
        // rls-allow: busca global respeita escopo via RLS
        const orderQuery = supabase
          .from('orders')
          .select('id, order_number, status, total, client_name, organization_id')
          .limit(5);
