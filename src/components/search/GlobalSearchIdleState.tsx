...
function SectionHeader({ icon, label, count, gradient }: {
  ...
  return (
    <div className="flex items-center gap-3 px-4 pt-5 pb-2.5">
      <div className={cn("h-6 w-6 rounded-2xl flex items-center justify-center shrink-0", gradient || "bg-primary/10")}>
...
function NavCard({ action, index, onSelect }: {
  ...
  return (
    ...
    >
      ...
      {action.shortcut && (
        <kbd className="hidden md:inline-flex h-5 min-w-[22px] items-center justify-center rounded-2xl bg-primary/10 border border-primary/20 px-1.5 font-mono text-[10px] font-semibold text-primary/60">
          {action.shortcut}
        </kbd>
      )}
            ...
            {history.slice(0, 4).map((term, i) => (
              ...
              >
                ...
                <button
                  ...
                  className="opacity-0 group-hover:opacity-100 group-data-[selected=true]:opacity-100 h-7 w-7 flex items-center justify-center hover:bg-destructive/10 rounded-2xl transition-all"
                >
          ...
          <div className="space-y-1 px-2">
            {popularProducts.map((product, idx) => (
              ...
              >
                ...
                {idx === 0 ? (
                  <Badge className="shrink-0 text-[10px] h-6 rounded-2xl bg-gradient-to-r from-orange/20 to-orange/10 text-orange border-orange/20 hover:bg-orange/25 font-semibold shadow-sm shadow-orange/10">
                    🔥 Top 1
                  </Badge>
                ) : (
