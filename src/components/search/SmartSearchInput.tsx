...
export const SmartSearchInput = forwardRef<HTMLDivElement, SmartSearchInputProps>(
  function SmartSearchInput(
    ...
    const showDropdown =
      isFocused &&
      (suggestions.length > 0 || quickSuggestions.length > 0 || searchHistory.length > 0);
                      ...
                      <div className="p-2">
                        ...
                        {searchHistory.slice(0, 5).map((item, index) => (
                          <motion.div
                            ...
                            className="group flex cursor-pointer items-center gap-3 rounded-xl px-3 py-2 transition-colors hover:bg-muted"
                            onClick={() => {
                              handleSelectResult({
                                type: 'history',
                                id: item.id,
