    ...
    <>
      ...
      <div className="flex items-center gap-2 w-full md:w-auto">
        ...
        >
          ...
          <div className="relative h-6 w-6 rounded-2xl bg-primary/8 group-hover:bg-primary/12 flex items-center justify-center transition-colors duration-300 shrink-0">
            <Brain className="h-3.5 w-3.5 text-primary/60 group-hover:text-primary transition-colors duration-300" />
            <div className="absolute inset-0 rounded-2xl bg-primary/10 animate-[brain-glow_3s_ease-in-out_infinite] pointer-events-none" />
          </div>
          ...
          <kbd className="hidden sm:inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-2xl border border-border/50 bg-muted/50 text-[10px] font-medium text-muted-foreground/60 group-hover:border-primary/20 group-hover:text-primary/50 transition-colors shrink-0">⌘K</kbd>
        </button>
        <Tooltip>
        ...
        <CommandList className="max-h-[520px] scrollbar-thin px-1 [background-color:hsl(var(--command-surface))]">
          ...
          {s.searchIntent && !s.isSearching && s.results.length > 0 && (
            <div className="flex flex-wrap items-center gap-2 px-4 py-3 mx-2 mt-3 rounded-xl border [border-color:hsl(var(--command-border))] [background:linear-gradient(90deg,hsl(var(--command-surface-raised)),hsl(var(--command-surface)))] animate-in fade-in-0 slide-in-from-top-1 duration-200">
              <div className="h-6 w-6 rounded-2xl bg-primary/12 flex items-center justify-center"><Brain className="h-3.5 w-3.5 text-primary" /></div>
              ...
              {s.searchIntent.type !== "mixed" && typeConfig[s.searchIntent.type] && (
                <Badge variant="outline" className="text-[11px] h-5.5 rounded-2xl font-semibold [border-color:hsl(var(--command-border-strong))] [background-color:hsl(var(--command-accent))]">
                  {typeConfig[s.searchIntent.type].label}s
                </Badge>
              )}
              {s.searchIntent.filters.category && <Badge variant="secondary" className="text-[11px] h-5.5 rounded-2xl [background-color:hsl(var(--command-accent))] [color:hsl(var(--command-text-muted))]">{s.searchIntent.filters.category}</Badge>}
              {s.searchIntent.filters.color && <Badge variant="secondary" className="text-[11px] h-5.5 rounded-2xl [background-color:hsl(var(--command-accent))] [color:hsl(var(--command-text-muted))]">Cor: {s.searchIntent.filters.color}</Badge>}
              {s.searchIntent.filters.priceRange && <Badge variant="secondary" className="text-[11px] h-5.5 rounded-2xl [background-color:hsl(var(--command-accent))] [color:hsl(var(--command-text-muted))]">{{ low: "Preço baixo", medium: "Preço médio", high: "Premium" }[s.searchIntent.filters.priceRange]}</Badge>}
              {s.searchIntent.filters.status && <Badge variant="secondary" className="text-[11px] h-5.5 rounded-2xl [background-color:hsl(var(--command-accent))] [color:hsl(var(--command-text-muted))]">Status: {s.searchIntent.filters.status}</Badge>}
              {s.searchIntent.filters.clientName && <Badge variant="secondary" className="text-[11px] h-5.5 rounded-2xl [background-color:hsl(var(--command-accent))] [color:hsl(var(--command-text-muted))]">Cliente: {s.searchIntent.filters.clientName}</Badge>}
            </div>
          )}
          ...
          {!s.isSearching && s.query.length >= 1 && s.query.length < 3 && (
            <div className="flex items-center justify-center gap-2.5 px-4 py-8 animate-in fade-in-0 duration-200">
              <div className="h-7 w-7 rounded-2xl [background-color:hsl(var(--command-accent))] flex items-center justify-center">
              ...
              <CommandGroup key={type} heading={config.label + "s"} className="animate-in fade-in-0 slide-in-from-bottom-2 duration-300 [&_[cmdk-group-heading]]:px-4 [&_[cmdk-group-heading]]:pt-4 [&_[cmdk-group-heading]]:pb-2">
                {items.map((result, i) => (
                  ...
                  >
                    ...
                    <Badge variant="outline" className="shrink-0 text-[10px] h-5 rounded-2xl [border-color:hsl(var(--command-border-strong))] [background-color:hsl(var(--command-accent))] font-medium">{config.label}</Badge>
        ...
        <div className="flex items-center justify-between px-5 py-2.5 border-t [border-color:hsl(var(--command-border))] [background:linear-gradient(90deg,hsl(var(--command-surface-raised)),hsl(var(--command-surface)),hsl(var(--command-surface-raised)))] select-none">
          <div className="flex items-center gap-4 text-[11px] [color:hsl(var(--command-text-subtle))]">
            <span className="inline-flex items-center gap-1.5">
              <kbd className="inline-flex items-center justify-center h-[18px] min-w-[20px] rounded-2xl [background-color:hsl(var(--command-accent))] border [border-color:hsl(var(--command-border))] font-mono text-[10px] leading-none px-1">↵</kbd>
            ...
            <span className="hidden md:inline-flex items-center gap-1.5">
              <kbd className="inline-flex items-center justify-center h-[18px] min-w-[20px] rounded-2xl [background-color:hsl(var(--command-accent))] border [border-color:hsl(var(--command-border))] font-mono text-[10px] leading-none px-1">1-9</kbd>
              <span>Saltar</span>
            </span>
            <span className="hidden md:inline-flex items-center gap-1.5">
              <kbd className="inline-flex items-center justify-center h-[18px] rounded-2xl [background-color:hsl(var(--command-accent))] border [border-color:hsl(var(--command-border))] font-mono text-[10px] leading-none px-1">⌘↵</kbd>
              <span>Nova aba</span>
            </span>
            <span className="inline-flex items-center gap-1.5">
              <kbd className="inline-flex items-center justify-center h-[18px] min-w-[20px] rounded-2xl [background-color:hsl(var(--command-accent))] border [border-color:hsl(var(--command-border))] font-mono text-[10px] leading-none px-1">ESC</kbd>
              <span>Fechar</span>
            </span>
          </div>
          <div className="flex items-center gap-1.5 text-[10px] text-primary/40 font-medium">
            <div className="h-4 w-4 rounded-2xl bg-primary/8 flex items-center justify-center"><Brain className="h-2.5 w-2.5" /></div>
