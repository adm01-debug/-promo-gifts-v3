...
import { Package, ArrowUpDown, Building2, FolderTree, X, RefreshCw, Search, CheckSquare, Loader2 } from "lucide-react";
...
export function ReplenishmentProductGrid() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    filtered.sort((a, b) => {
      switch (sortMode) {
        ...
        case "newest": default: return new Date(b.replenished_at).getTime() - new Date(a.replenished_at).getTime();
  ...
  if (error) console.error('Erro ao carregar reposições:', error);
  ...
  const productMap = useMemo(() => {
    ...
    filteredProducts.forEach(n => map.set(n.product_id, replenishmentToProduct(n)));
    return map;
  }, [filteredProducts]);
    ...
    if (filteredProducts.length === 0) {
      return (
        <div className="text-center py-10">
          ...
          <p className="text-muted-foreground font-medium text-sm">{hasActiveFilters ? "Nenhuma reposição com esses filtros" : "Nenhuma reposição encontrada"}</p>
          {hasActiveFilters ? <Button variant="link" className="mt-1 text-xs" onClick={clearFilters}>Limpar filtros</Button> : <p className="text-xs text-muted-foreground/70 mt-1">Produtos repostos aparecerão aqui automaticamente</p>}
        </div>
      );
    }
    if (viewMode === "table") return <ReplenishmentTableView products={filteredProducts} onProductClick={handleProductClick} selectionMode={selectionMode} selectedIds={sel.selectedIds} onToggleSelect={sel.toggleSelect} />;
      ...
      return (
        <div className="space-y-2">
          {filteredProducts.map((item, index) => {
            ...
            return (
              <div key={item.replenishment_id} className="stagger-item" style={{ animationDelay: `${Math.min(index * 25, 250)}ms` }}>
                <div className={cn("flex items-center gap-1", isSelected && "ring-2 ring-primary rounded-xl")}>
                  {selectionMode && (
                    <div className="flex-shrink-0 ml-1">
                      <SelectionCheckbox checked={isSelected} onChange={() => sel.toggleSelect(item.product_id)} size="md" />
                    </div>
                  )}
    ...
    return (
      <div className={`grid ${getGridColsClass(effectiveCols)} ${getGridGapClass(effectiveCols)}`}>
        {filteredProducts.map((product, index) => (
          <div key={product.replenishment_id} className="stagger-item" style={{ animationDelay: `${Math.min(index * 25, 250)}ms` }}>
  ...
  return (
    <div className="space-y-3">
          ...
          <div className="flex items-center gap-2 flex-1 min-w-0">
            ...
            <div className="hidden sm:block w-80 lg:w-[25rem]">
              <div className="relative">
                ...
                <Input placeholder="Buscar reposições…  /" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="h-8 text-xs pl-8 bg-muted/40 border-border/50 focus:bg-background" />
      ...
      {!isLoading && filteredProducts.length > 0 && hasActiveFilters && (
        <p className="text-[11px] text-muted-foreground">Mostrando <span className="font-medium text-foreground">{filteredProducts.length}</span> de {products.length} reposições</p>
      )}
