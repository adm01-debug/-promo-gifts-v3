...
export function KitComparisonDialog({ open, onOpenChange, kits }: KitComparisonDialogProps) {
  ...
  const getTotalWeight = (kit: KitForComparison): number => {
    ...
    const itemsWeight = (kit.items_data || []).reduce(
      (sum: number, i: any) => sum + (i.weight || 0) * (i.quantity || 1),
      0,
    );
  ...
  const getItemCount = (kit: KitForComparison): number => {
    return (kit.items_data || []).reduce((sum: number, i: any) => sum + (i.quantity || 1), 0);
  };
  ...
  const rows = [
    {
      ...
      getValue: (k: KitForComparison) =>
        ({ montado: 'Montado', original: 'Original', simples: 'Simples' })[k.kit_type] ||
        k.kit_type,
    },
    {
      label: 'Embalagem',
              ...
              {rows.map((row, idx) => (
                <tr key={idx} className={idx % 2 === 0 ? 'bg-muted/30' : ''}>
                  ...
                  {kits.map((kit) => (
                    <td
                      key={kit.id}
                      className={`px-2 py-2.5 text-center ${(row as Record<string, unknown>).bold ? 'font-bold text-primary' : ''}`}
                    >
                      {row.getValue(kit)}
                    </td>
                  ))}
