...
import { formatCurrency, generatePriceBreakdown, calculateTotalKitPrice } from "@/lib/kit-builder";
...
export function KitPricingCard({ kitState, kitQuantity, onKitQuantityChange }: KitPricingCardProps) {
    ...
    <>
      <Card>
        ...
        <CardContent>
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm"><span>Produtos ({kitQuantity} {kitQuantity === 1 ? 'kit' : 'kits'})</span><span>{formatCurrency(pricing.subtotal)}</span></div>
            {pricing.personalizationPrice > 0 && <div className="flex justify-between text-sm text-primary"><span>Personalização</span><span>{formatCurrency(pricing.personalizationPrice)}</span></div>}
          </div>
          <Separator className="my-4" />
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base flex items-center gap-2"><TrendingUp className="h-4 w-4 text-primary" />Preço por Quantidade</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-5 gap-2 text-center">
            {[50, 100, 200, 500, 1000].map(qty => {
              ...
              return (
                <button key={qty} onClick={() => onKitQuantityChange(qty)} className={cn("rounded-2xl p-2.5 border transition-all cursor-pointer", isCurrentQty ? "border-primary bg-primary/10 ring-1 ring-primary" : "border-border/50 bg-secondary/30 hover:border-primary/30")}>
