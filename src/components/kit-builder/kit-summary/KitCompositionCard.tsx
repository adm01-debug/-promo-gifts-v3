...
import { formatCurrency, formatDimensions, formatVolume } from "@/lib/kit-builder";
...
export function KitCompositionCard({ kitState, kitQuantity, stockByProduct }: KitCompositionCardProps) {
  ...
  return (
    <Card>
      ...
      <CardContent>
        {box && (
          <div className="flex items-center gap-3 p-3 rounded-2xl bg-secondary/50 mb-3">
            <div className="w-12 h-12 rounded-2xl bg-background overflow-hidden">
        ...
        <div className="space-y-2">
          {items.map(item => {
            ...
            return (
              <div key={item.id} className="flex items-center gap-3 p-2 rounded-2xl hover:bg-secondary/30">
                <div className="w-10 h-10 rounded-2xl bg-secondary overflow-hidden">
