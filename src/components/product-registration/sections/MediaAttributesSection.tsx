...
export function MediaAttributesSection({ form, referenceData }: MediaAttributesSectionProps) {
  ...
  const setPrimary = (index: number) => {
    form.setValue(
      'images',
      form.getValues('images').map((img, i) => ({ ...img, is_primary: i === index })),
    );
  };
  ...
  const toggle = (field: 'colors' | 'materials' | 'technique_ids', id: string) => {
    ...
    form.setValue(
      field,
      current.includes(id) ? current.filter((v: string) => v !== id) : [...current, id],
    );
  };
    ...
    <>
                  ...
                  <CardContent className="p-2">
                    ...
                    <div className="mt-2 flex items-center justify-between">
                      <Button
                        ...
                        className={cn('text-xs', img.is_primary && 'text-primary')}
                      >
      ...
      <FormSection title="Cores Disponíveis" required description="Selecione as cores disponíveis">
        ...
        {form.formState.errors.colors && (
          <p className="mt-2 text-sm text-destructive">{form.formState.errors.colors.message}</p>
        )}
      </FormSection>
      ...
      <FormSection title="Materiais" required description="Selecione os materiais do produto">
        ...
        {form.formState.errors.materials && (
          <p className="mt-2 text-sm text-destructive">{form.formState.errors.materials.message}</p>
        )}
      </FormSection>
      ...
      <FormSection title="Técnicas de Personalização" collapsible defaultOpen={false}>
        <div className="flex flex-wrap gap-2">
          {referenceData.techniques.map((t) => (
            <Badge
              key={t.id}
              variant={selectedTechniques.includes(t.id) ? 'default' : 'outline'}
              className="cursor-pointer"
              onClick={() => toggle('technique_ids', t.id)}
            >
              {t.name}
            </Badge>
          ))}
