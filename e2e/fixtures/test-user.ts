 ...
 *
 * Usar este helper sempre que o teste criar algo nomeável (orçamento,
 * coleção, lista, kit, etc.) facilita auditoria e limpeza pontual.
 */
export function e2eName(label: string): string {
  const ts = Date.now();
  const rand = Math.random().toString(36).slice(2, 6);
  return `${getTestPrefix()} ${label} ${ts}-${rand}`;
}
...
export function isE2eName(name: string | null | undefined): boolean {
  ...
  return name.startsWith(getTestPrefix());
}
