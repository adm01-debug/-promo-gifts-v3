...
Deno.test({ name: "CROSS: all 10 critical tables have RLS enabled", sanitizeOps: false, sanitizeResources: false, fn: async () => {
  for (const t of ["profiles","user_roles","quotes","orders","order_items","organizations","organization_members","admin_audit_log","quote_items","quote_approval_tokens"]) {
...
Deno.test({ name: "9. quote_approval_tokens: full CRUD scoped to seller_id", sanitizeOps: false, sanitizeResources: false, fn: async () => {
  const p = await getPolicies("quote_approval_tokens");
  const cmds = p.map(x => x.cmd);
  for (const req of ['SELECT','INSERT','UPDATE','DELETE']) {
    assert(cmds.includes(req), `Must have ${req} policy`);
  }
  for (const pol of p) {
    assert(`${pol.qual??''} ${pol.with_check??''}`.includes("seller_id"), `"${pol.polname}" must check seller_id`);
  }
}});
