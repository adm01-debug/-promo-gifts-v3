...
CREATE TABLE public.color_variations (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE public.color_nuances (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    name VARCHAR(100) NOT NULL,
    slug VARCHAR(100) NOT NULL UNIQUE,
    description TEXT,
...
CREATE INDEX idx_color_variations_group ON color_variations(group_id);
CREATE INDEX idx_color_groups_active ON color_groups(is_active) WHERE is_active = true;
CREATE INDEX idx_color_variations_active ON color_variations(is_active) WHERE is_active = true;
CREATE INDEX idx_color_nuances_active ON color_nuances(is_active) WHERE is_active = true;
